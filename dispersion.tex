...
In order to implement these operations efficiently, we store the polylines in augmented balanced search trees, so we can answer queries in logarithmic time. Each node of the trees will store $d(h,v)$ as its key, and $W(P)$ for this key. The breakpoints will be represented at the leaves of the tree, and the key and value of each leaf will be the sum of all its ancestors. Querying the value at some point of the polyline can be done by a predecessor query. Shifting the polyline can be done by increasing the key at the root of the augmented tree. Increasing the value for some interval can be done by going up to the LCA of the endpoints of the interval, and adding to the value there. The negation needed for the "mirror" operation can be done by saving an extra bit in all nodes of the tree. Finding the last breakpoint in some interval of a polyline can also be done by a predecessor query.
