...
The algorithm we present computes such a polyline for the subtrees rooted at all the nodes of the tree. As we mentioned, we will compute this bottom-up, and so in each step of the algorithm we compute the polyline for a subtree rooted at some node $v$, after we have already computed the polylines for the subtrees rooted at his children. When constructing a polyline, we need to consider several cases, as the closest chosen node in the subtree could be the root, or it could be in the subtree of one of its children. This means that for any key $d(h,v)$ in the polyline, the value for that key can represent a solution $P$ in which $h$ is in the subtree rooted $v$'s left child, or in the subtree rooted $v$'s right child, or it could be $v$ itself.\footnote{For the same reasoning as in the unweighted case, we assume w.l.o.g. that the input tree is binary.}
...
Operation 7 - batched value predecessor: We store in each node the minimum value in its subtree. Each value predecessor query will be answered by first finding $k_i$ (or it's predecessor, if there is no breakpoint at $k_i$), then going up the tree until the minimum value in the current node's subtree is smaller or equal to the value of the starting point, and then going down while searching for $v_i$. Notice that since the polyline is monotone decreasing, the leaves are sorted by keys and by values, and so we can search in the tree by keys or by values. Each predecessor key takes $O(\log y)$ and so the operation takes $O(x \log y)$ in total.
...
We prove by induction that for any binary tree with $n$ nodes, $S \leq \log (n!)$. It ease easy to see that the base of the induction holds. Assume that for any binary tree with $k$ nodes, where $k<n$, it holds that $S \leq \log (k!)$. Consider some binary tree with $n$ nodes. If the root of the tree has only one child, $S$ is the same for the subtree rooted at the child, and so $S \leq \log ((n-1)!) \leq \log (n!)$. If the root has two children, denote the size of its left subtree by $n_1$ and the size of its right subtree by $n_2$. Assume w.l.o.g. that $n_1 \leq n_2$. From the induction hypothesis it follows that $S \leq n_1 + \log (n_1!) + \log (n_2!)$. We would like to prove that $n_1 + \log (n_1!) + \log (n_2!) \leq \log(n!)$. Let us expand both parts of the inequality:
\begin{dmath}
n_1 + \log (n_1!) + \log (n_2!) \leq \log (n!) \Rightarrow
\log (2^{n_1}) + \log (n_1!) + \log (n_2) + \log (n_2 - 1) + \log (n_2 - 2) +...+ \log (2) \leq \log (n) + \log (n-1) + \log (n-2) +...+ \log (2) 
\Rightarrow \log (2^{n_1} \cdot n_1!) \leq \log (n) + \log(n-1) +...+ \log(n_2+1) \Rightarrow
\log (2n_1)+\log (2(n_1-1))+...+\log (2) \leq \log (n) + \log(n-1) +...+ \log(n_2-1)
\end{dmath}
The last inequality holds, because there are $n_1$ elements in each side of it, and we know that $n_1 \leq \frac{n}{2}$.

Since $\log (n!) = O(n \log n)$, we get a total comlexity $O(n \log n \cdot \log n) = O(n \log ^2 n)$ for our weighted feasibility test.
\end{proof}
