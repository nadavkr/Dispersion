...
Finally, in Section~\ref{section:weighted} we consider the weighted case. Our main result  is an  $O(n\log n)$ feasibility test (for which we also give a tight lower bound). In contrast to the unweighted case where each node had only one candidate, in the weighted case each  node can have a large number of candidates. To overcome this, we describe the candidates of every node with a monotonically decreasing polyline. We then show that the polyline of a node can be efficiently computed from the polylines of its children. To this end, we develop a rather intricate data structure (see Section~\ref{section:data structure}) that can efficiently manipulate polylines with various batch updates. We believe this data structure is of independent interest.  
%TODO: Say that our data structure is a simplification/extension/etc. of Tarjan's.  
...
Let  ${M_l, M_2, . . . , M_N}$ be a collection of sorted matrices in which matrix $M_j$ is of dimension $m_j \times n_j$, $m_j \leq n_j$, and $\sum_{j=1}^{N} m_j = m$.
