...
Again, we split the tree into small trees with $O(\frac{y}{x})$ leaves. Additionally, for each list of breakpoints that should be inserted between a pair of consecutive breakpoints stored in the same small tree, we split that small tree between these consecutive breakpoints. This can be done by scanning the list of breakpoints to be inserted from left to right, while scanning the roots of the small trees. Since we have at most $x$ such extra splits, each of which takes $O(\log(\frac{y}{x}))$ time, the total time for all splits is $O(x\log(\frac{2y}{x}))$ in the worst case. We now have $O(x)$ small trees with $O(\frac{y}{x})$ leaves, such that every breakpoint
