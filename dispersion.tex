...
\begin{proof}
Notice that the number of breakpoints in a polyline representing some subtree, is at most the number of nodes in the same subtree. We split the analysis into two parts: first we consider nodes of degree one, and then nodes of degree two.
For any node with degree one, constructing the polyline for its subtree, given the polyline for the subtree rooted at its child takes logarithmic time, i.e. no more than $O(\log n)$. Since we have at most $n$ nodes with degree 1, the total processing time for these nodes is $O(n \log n)$. 
As we mentioned, operations 1-5 only take logarithmic time per step, while operations 6-9 take $O(x \log y)$ time.  Thus, constructing the polyline for the subtree rooted at some node $v$, with degree 2, cost $O(x \log y)$ time, where $x$ and $y$ are the sizes of the subtrees rooted at the children of $v$, and $x \leq y$. Clearly it holds that $x \log y \leq x \log n$. %TODO finish
\end{proof}
