...
\subsection{\boldmath$ \Omega (n\log n)$ lower bound for the weighted feasibility test}

To prove a lower bound on the weighted feasibility test, we reduce from the set disjointness problem:
...
Without loss of generality, the elements of $X$ and $Y$ are non-negative, as otherwise
...
\begin{proof}
We start by proving that if the sets are not disjoint, then there is a subset of the vertices of the tree, $P \subseteq T$, s.t. $W(P) \geq K$ and $f(P) \geq K$ (where $W(P)$ is the weight of $P$, i.e. the sum of the weights of all the vertices in $P$). Assume $x_i=y_j$. Then, $d(x_i,y_j) = d(x_i,u) + d(u,v) + d(v,y_j) = \frac{K}{2} - x_i -1 + \frac{K}{2} + y_j +1 = K \geq K$. $W(x_i) + W(y_j) = x_i + 1 + K- y_j -1 = K \geq K$, and so the feasibility test should return true with $P = \lbrace x_i, y_j \rbrace$.
...
\subsection{\boldmath$O(n \log ^2 n)$ Algorithm for the weighted feasibility test}
Similarly to the unweighted case, we would like to compute for each node of the tree, the subset of nodes in its subtree, $P$, s.t. $f(P) \geq \lambda$ and $W(P)$ is maximized. We compute this by going over the nodes of the tree bottom-up. Previously, the situation was simpler, as we had just one candidate node that we were not sure would be in the solution for the whole input tree. This was true, because only a node that is closer to the root could be at distance less than $\lambda$ from a certain node, and we could always take the node that is farthest from the root between the two, since nodes were of uniform weight.
Now, we must have a solution ($P$) inside the subtree for any possible distance of the closest chosen node outside the subtree (up to $\lambda$, since if the closest node is farther, it does not affect nodes inside the subtree). This can be viewed as a monotone polyline, since the size of $P$ only increases as the distance of the closest node (denoted by $d(c)$) increases from zero to $\lambda$. The size of $P$ only changes at certain points, i.e. the breakpoint of the polyline. We store these polylines in binary search trees, so we can answer queries in logarithmic time, using predecessor queries. Each node of a binary search will store $d(c)$ as its key, and the size of $P$ when this is the distance of the closest chosen node.  %TODO add figure

We now present a single step of the algorithm. Denote the root of the subtree by $v$, its left child by $u_1$, and its right child by $u_2$. We have two polylines that represent the solutions inside the subtrees rooted at $u_1$ and $u_2$, and we would like to create the polyline for the subtree rooted at $v$.
Assume w.l.o.g. that the number of breakpoints in $u_1$'s polyline (denoted by $p_1$), is greater or equal to the number of breakpoints in $u_2$'s polyline (denoted by $p_2$). We consider three cases, for each of them construct a polyline, and then merge them into one polyline.
\begin{enumerate}
\item \emph{$v$ is in the solution for its subtree: } In this case we simply query $p_1$ and $p_2$ for their values with distances of the closest node being $d(v,u_1)$ and $d(v,u_2)$, add them together with the weight of $v$, and get the size of the solution for $v$'s subtree in case $v$ is in the solution.
