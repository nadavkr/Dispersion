...
A \textit{centroid} node of a tree, is a node that divides the tree into pieces s.t. no piece has more than half of the nodes of the entire tree. The centroid decomposition decomposes the tree by recursively splitting the tree into pieces using the centroid node .
\end{definition}
%TODO add drawing
...
Notice that throughout our algorithm we process caterpillars, and do not consider taking spine nodes to our solution. This is fine, since we can add some artificial nodes with zero-weighted edges. This is also how we would handle non-binary trees. Given an input tree which is not binary, we can separate each node of degree higher than three, to several nodes with degree at most three (with zero-weighted edges between them). These artificial nodes do not pose a problem, as they will be spine nodes that cannot be chosen.
