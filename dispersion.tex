...
We first describe how to solve the feasibility test in linear time, and then use this algorithm to solve the dispersion optimization problem.
...
We are given a root vertex $v$, its children nodes $v_{1},v_{2},...,v_{k}$, and for each child we are given a maximal valid solution for the feasibility test on its subtree. We would like to produce a maximal valid solution for the feasibility test on $v\text{'s subtree}$.
Denote by $P_{1},...,P_{k}$ the solutions for the feasibility test on each subtree rooted at a child of $v$.
For any subtree of $T$ rooted at node $r$, and a valid solution $P$ for a feasibility test on the subtree, we call a node $u\in P$, s.t. $d(r,u)<\frac{\lambda}{2}$, a \emph{candidate} node of the subtree. We call the vertex in $P$ that is closest to $r$, but isn't a candidate, \emph{certain}.
...
\item Take all candidate nodes $u$ s.t. $d(u,v) \geq \frac{\lambda}{2}$.
\item Take $u'$, the candidate node farthest from $v$ s.t. $d(u',v) < \frac{\lambda}{2}$, if it exists, and if $d(u',x)\geq \lambda$, where $x$ is the closest node to the root $v$ we have chosen so far.
...
Clearly it holds that $f(P) \geq \lambda$ for the set $P$ that the algorithm outputs. Assume for contradiction that $P$ is not maximal. Denote by $r$ the lowest vertex of the tree where the algorithm does not output a maximal valid subset for the subtree rooted at $r$. Due to Lemma \ref{greenNodesLemma}, there is a maximal valid solution that includes all the vertices in $P$ that are at distance greater or equal to $\frac{\lambda}{2}$ from $r$. Denote this maximal solution by $P'$. $P'$ cannot contain a node at distance greater or equal to $\frac{\lambda}{2}$ from $r$, that is not in $P$, as that is a contradiction to the maximality of the solutions for the children of $r$. Aside from these vertices, $P$ contains at most one more vertex, which is at distance less than $\frac{\lambda}{2}$ from $r$. If $P$ does not contain such a vertex, $P'$ must contain one. This is a contradiction, since if there was such a vertex that was at distance at least $\lambda$ from all the nodes we have chosen so far, it would also be in $P$. If $P$ does contain a node at distance less than $\frac{\lambda}{2}$ from $r$, then $P'$ contains at least two such nodes, which is impossible of course.
...
We now would like to pre-process the fragments s.t. we can later check in constant time for any two nodes in a fragment, $u_1,u_2$ if $d(u,v)\geq\lambda$, and if $d(u_1,u_2) \leq \frac{\lambda}{2}$, for any possible $\lambda$. The possible values of $\lambda$ in the beginning are of course all the pairwise distances in the input tree. We achieve the ability to perform such queries for most of the fragments, by using a parametric search method by Frederickson that will allow us to eliminate some of the possible values of $\lambda$ by using our existing feasibility test. By the end of this elimination process, most of the fragments will have the property that any pairwise distance inside them is either smaller then the smallest possible value of $\lambda$ or
...
\subsubsection{Feasibility Test} %TODO check if we can simplify
