...
The idea is partitioning the tree to fragments and preprocessing them, so that in query time (for a feasibility test query) we can "skip" over most fragments in $O(1)$.
\paragraph{The Tree Partitioning} We would like to have fragments of size $b=log^2n$, s.t each fragment will be connected to the rest of the tree by at most two vertices. Each fragment have at most $b$ vertices, and will be defined by its border vertices $u$ and $v$ s.t $v$ is a descendant of $u$, as in the figure. We call the path from $u$ to $v$, the fragment's \textit{spine}, and $v$'s subtree the \textit{hole} of the fragment. %TODO add figure
\begin{lemma}
For any binary tree on n nodes and a parameter b there is a
partition of the tree into $O(n/b)$ fragments of size at most b.
\end{lemma}
\begin{proof}
Call a node $u$ \textit{large} if the size of its subtree is greater than b and \textit{small}
otherwise. Consider the subtree consisting of only large nodes. For
each leaf in that subtree, make each of its children a new fragment
with no holes. Then, for every branching node in that subtree, make it
a fragment consisting of just one node. Remove all already created
fragments from the tree. The remaining nodes correspond to paths
consisting of large nodes in the original tree with some small
subtrees possibly attached to them (in other words: unary chains in
the subtree). Greedily partition each such path into fragments of size
<= b by traversing bottom-up. Essentially, there will be at most one
"extra" fragment per such path, but the number of paths is O(n/b), so
O(n/b) fragments in total.
\end{proof}

We partition the tree into fragments of size $b=log^2n$. The fragments
have structure as described in the quoted email, i.e., at most one
hole in each of them (maybe no holes at all in some of them, also
maybe u=v and the fragment is just a single node). It means that every
fragment is specified by two nodes u, v such that u is an ancestor of
v, path u - v will be called its spine (for obvious reasons).

Lemma. For any binary tree on n nodes and a parameter b there is a
partition of the tree into O(n/b) fragments of size <= b.
Proof: Call a node u large if the size of its subtree is > b and small
otherwise. Consider the subtree consisting of only large nodes. For
each leaf in that subtree, make each of its children a new fragment
with no holes. Then, for every branching node in that subtree, make it
a fragment consisting of just one node. Remove all already created
