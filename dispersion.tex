...
\begin{case}
\textbf{$d(r,v) < \frac{\lambda}{2}$:} Because of the monotonicity of the distances of leaves from the root, every other leaf of the caterpillar is also at distance less than $\frac{\lambda}{2}$ from $r$. If $v$ is farther from $r$ than $u$ (which is possible, because the monotonicity only holds inside each fragment), or if $u$ is certain w.r.t $r$, we set $v$ to be the new candidate node (since it would also be farther from any node in the hole than $u$). If this is not the case, we leave $u$ as the candidate node and move up to the next fragment without taking any nodes.
\end{case}
\begin{case}
\textbf{$d(r,v) \geq \frac{\lambda}{2}$:}
In this case we need to consider the suffixes of the current fragment. We compute the suffix of the fragment that cannot be taken. We can do this by binary searching for the first node of the fragment that is at distance at least $\lambda$ from the current candidate node. We have pre-computed all we need to know once we have the relevant prefix.
\end{case}
...
\item\textbf{Construct a row and column sorted matrix storing all pairwise distance in the caterpillar}
