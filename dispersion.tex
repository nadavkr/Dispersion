...
Finally, in Section~\ref{section:weighted} we consider the weighted case. Our main result  is an  $O(n\log n)$ feasibility test (for which we also give a tight lower bound). In contrast to the unweighted case where each node had only one candidate, in the weighted case each  node can have a large number of candidates. To overcome this, we show that the candidates of every node can be captured by a monotone polyline. We then show that the polyline of a node can be computed from the polylines of its children. To this end, we develop a rather intricate data structure (see Section~\ref{section:data structure}) that can efficiently manipulate the polylines with various batch updates. We believe this data structure is of independent interest.  
...
{\bf Operations~\ref{op1} and~\ref{op2}} can use standard split and merge procedures for 2-3 trees. This will take logarithmic time. 
{\bf Operation~\ref{op3}} can be done by running a predecessor query, and returning the value at the returned breakpoint. 
{\bf Operation~\ref{op4}} is done by an inorder traversal of the tree. 
