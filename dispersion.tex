...
\section{Overview}
Before presenting our optimal linear algorithm for the dispersion optimization problem, we first present $O(n\log \log n)$ and $O(n\log ^*n)$ algorithms. Each of these algorithms builds on the previous, and uses some new ideas to improve the complexity. All three algorithms are based on the sublinear algorithm we present for the feasibility test in Section \ref{sublinear f.t.}. This algorithm uses the simple linear algorithm presented in Section \ref{linear F.T.}, which is similar to the one presented in \cite{Bhattacharya1991}. We use a new technique for partitioning trees, in order to preprocess the resulting fragments in linear time, s.t. we are able to run feasibility tests in sublinear time. This partitioning is similar to the micro-macro decomposition, but our partitioning scheme takes only linear time overall for partitioning the tree into fragments of increasing sizes, which allows us to use a bootstrapping technique in linear time. %TODO add citation for micro-macro
