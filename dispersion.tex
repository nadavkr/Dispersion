...
Finally, in Section~\ref{section:weighted} we consider the weighted case. Our main result  is an  $O(n\log n)$ feasibility test (for which we also give a tight lower bound). In contrast to the unweighted case where each node had only one candidate, in the weighted case each  node can have a large number of candidates. To overcome this, we show that the candidates of every node can be captured by a monotone polyline. We then show that the polyline of a node can be computed from the polylines of its children. To this end, we develop a rather complicated data structure that can efficiently manipulate the polylines with various batch updates. We believe this data structure is of independent interest.  
%TODO: Say that our data structure is a simplification/extention/etc. of Tarjan's.  
...
Out of all the already chosen nodes in the subtree rooted at $v$, let $h$ be the node for which $d(h,v)$ is minimized. We call $h$ the {\em closest chosen node} in $v$'s subtree. For our weighted feasibility test, we will store a solution ($P$) inside $v$'s subtree for any possible distance of the closest chosen node (up to $\lambda$, since if the closest node is farther, it does not affect nodes outside the subtree). This can be viewed as a monotone polyline, since the weight of $P$ only decreases as the distance of the closest chosen node increases (from zero to $\lambda$). The weight of $P$ can only change at certain points, i.e. the breakpoints of the polyline. Each point of the polyline is a key-value pair, where the key is $d(h,v)$, and the value is $W(P)$ (the weight of the heaviest possible subset of nodes $P$ in $v$'s subtree  s.t. the closest chosen node is at distance greater or equal to $d(h,v)$ from $v$, and $d(x_1,x_2) \geq \lambda$ for every  $x_1,x_2\in P$).

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.4]{polyline}
\end{center}
\caption{The polyline represents the weight of the solution, $P$, as a function of the distance of the closest chosen node in the subtree. %$v$ is the root of the subtree, and $h$ is the chosen node closest to $v$ in its subtree. 
The weight of $P$ only decreases at certain points called breakpoints. Each breakpoint holds the value of the interval between itself and the next breakpoint.
...
If $v$ has two children, denote its left child by $u_1$ and its right child by $u_2$. We have two polylines that represent the solutions inside the subtrees rooted at $u_1$ and $u_2$ (denoted by $p_1$ and $p_2$ respectively), and we would like to create the polyline for the subtree rooted at $v$ (denoted by $p$).
...
Still, this process might result in a polyline which is not monotonically decreasing, because as we go over the intervals of $p_1'$ from left to right we increase their value by an increasing series of numbers (right to left in $p_2'$). See Figure~\ref{figure of constructing the first half of the polyline case 1}. %For instance assume that the next breakpoint after $bp_2$, is at $x+y+z$. Then, we would increase the value of $p_1'$ at $bp_2$ by the value of $p_2'$ at $\lambda-x-y-z$, which is at least the value by which we increased the previous interval (since $p_2'$ is monotone decreasing). 
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.9]{polyline_first_half_construction_case1}
\end{center}
...
\end{center}
\caption{Case II of constructing the first half of the polyline. The first half of $p_2$ is in blue. The vertical black dashed lines are the breakpoints of $p_1$. We lift the intervals of $p_2$ and get the red polyline, which is not monotone. To make it monotone, we delete all the breakpoints below the green intervals and get a monotone decreasing polyline.\label{figure of the second case in the construction of the first half of the polyline}
}
\end{figure}
%TODO: Why is the left endpoint of every green interval in the figure always a corner? In general it can by any point in the middle of a horizontal red line.
...
\item Batched value predecessor - Given a list of key-value pairs, $\{(k_i,v_i)\}$, find for each $k_i$, the maximal key $k_{i}'$, s.t. $k_{i}' \leq k_i$ and the value of the polyline at $k_{i}'$ is at least $v_i$.
...
We now show how to use some standard techniques to implement the required operations, and achieve $O(n \log ^2 n)$ time for our weighted feasibility test. %In the next section we will use results by Tarjan, that will allow us to analyze the cost of these operations more precisely, and prove that the overall running time of our algorithm is $O(n \log n)$.
...
{\bf Operations 1 and 2} can use standard split and merge procedures for 2-3 trees. This will take logarithmic time. 
{\bf Operation 3} can be done by running a predecessor query, and returning the value at the returned breakpoint. 
{\bf Operation 4} is done by an inorder traversal of the tree. 
...
In this section, we improve the complexity of our weighted feasibility test by implementing the interface we use for operations on polylines more efficiently. We achieve a $O(x \log (\frac{2y}{x}))$ time bound for the batched operations and show that the overall running time induced by this implementation is $O(n \log n)$, which matches the lower bound. For this we use the same augmented 2-3 trees described in the previous section, with some changes in the implementation of the batched operations. We now present $O(x \log(\frac{2y}{x}))$ time procedures for the four batched operations.

\paragraph{Operation 5 - batched interval increase:}
In the previous section we performed each interval increase by finding the interval's starting point, and traversing the path from it to the end of the interval, going through the LCA. This took $O(\log y)$ time since we performed the operations on a balanced tree with $y$ leaves. We would like to perform operations on smaller trees, in order to lower the complexity. We start by splitting the tree into $O(x)$ smaller trees, each of which has $O(\frac{y}{x})$ leaves. This is done by recursively splitting the tree, first to two trees with $O(\frac{y}{2})$ leaves, then we split each of these trees to two trees with $O(\frac{y}{4})$ leaves, and so on, until we have trees of size $O(\frac{y}{x})$. We then increase the values of the relevant intervals in the small trees. For this, we scan the roots of the small trees, searching for the starting point of the first interval. We can check whether the starting point is in some tree by using the maximum value field stored in its root. Once we have found the starting point of the interval, we check if the end of the interval is in the same tree or not (again, using the maximum value field). In the first case, the interval is contained in a single tree, and can be increased in this tree in time $O(\log(\frac{y}{x}))$, using the procedure described in the previous section. In the second case the interval spans over several trees, and so we need to do an interval increase in the two trees where the endpoints of the interval lie, and increase the value stored in the root of every tree that is entirely contained in the interval. We then continue to the next interval, and proceed in the same manner. Since we can check if the end of the interval is in some small tree in constant time, and since the given intervals are disjoint, the cost of the interval increases is the cost of at most two interval increases on small trees per interval (which take $O(\log (\frac{y}{x}))$ time, since this is the height of the small trees), plus the cost of scanning the roots of the trees. We have $O(x)$ small trees, and so the total cost for the interval increases on the small trees is $O(x \cdot \log (\frac{y}{x}) + x) = O(x \log (\frac{2y}{x}))$. We now merge the small trees back into one large tree using join operations, symmetrically to how we obtained the small trees.

Let us consider the time it takes to obtain the small trees with $O(\frac{y}{x})$ leaves. Assume we start with a tree that has $y$ leaves, split it at the middle element in $O(\log y)$ time, and obtain two trees with $\ceil{\frac{y}{2}}$ and $\floor{\frac{y}{x}}$ leaves. We then split these two trees, which takes $2 \cdot O(\log(\frac{y}{2}))$ time, and so on. The cost of this process sums up to:
\begin{align*}
...
& = O(x \log (\frac{2y}{x})) 
\end{align*}
Clearly, the cost of the joins we make in order to obtain the large tree in the end of the operation is not larger than the cost of the splits used to obtain trees with $O(\frac{y}{x})$ leaves, and so, the operation takes $O(x \log (\frac{2y}{x}))$ time in total.

\paragraph{Operation 6 - batched value predecessor:}
As in operation 5, we split the tree into smaller trees with $O(\frac{y}{x})$ leaves, and operate on them. We would like to perform the batch of value predecessor queries in the same way we did the interval increases, i.e. iterate over the given keys, and for each $k_i$ find its value predecessor $k_i'$, and continue to the next key on the list. For this we need the intervals defined by the pairs $(k_i',k_i)$ to be disjoint, as otherwise we might scan over some roots of small trees many times, and the operation would take $O(x \log(\frac{y}{x}) + x^2)$ time. But the intervals are not necessarily disjoint. 

Recall that we have two uses for the batched vale predecessor operation. The first is to prune the non-monotone polyline we got by increasing intervals of $p_2$ (the larger polyline) induced by the breakpoints of $p_1$ (see Figure \ref{figure of the second case in the construction of the first half of the polyline}), and the pruning is done by deleting the intervals returned by this operation. Consider two keys $k_1$ and $k_2$ s.t. $k_1 \leq k_2$, for which we would like to find value predecessor during the batched value predecessor operation. If the value of the polyline at $k_1$ (denoted by $v_1$) is greater or equal to the value of the polyline at $k_2$ (denoted by $v_2$), then certainly $k_1 \leq k_2'$, and the intervals are disjoint. Else, $v_1<v_2$, the intervals might not be disjoint, but we know that $k_1' \geq k_2'$, and so the interval $(k_1',k_1)$ is contained in the interval $(k_2',k_2)$. Thus, deleting the second interval is enough, meaning that we do not need to run the value predecessor query for $k_1$ at all. Generalizing this observation, we can see that we can remove elements from the input list of keys (which is given in sorted order, as it is the list of breakpoints of the second half of $p_1$) so that the values of the keys will be monotone decreasing. In other words, before running the value predecessor queries, we iterate over the input list of keys, and push each $k_i$ into a stack, only after popping all keys from the stack for which the corresponding value is less than $v_i$.

We can now run value predecessor queries for this pruned list of keys, using our small trees with $O(\frac{y}{x})$ leaves. We scan the small trees (from end to start), while searching for the maximal key in the input list, using the maximum key field. Once we find it, we run a value predecessor query (as described in the previous section) in the same small tree. If we find the value predecessor, we continue to the next key in the input list, and if not we must continue to scan the small trees. As in operation 5, we can check whether the answer is in the next tree by using the maximum value field in its root. Again, we search for the value predecessor of each key in at most two small trees, and the total cost of the operation is $O(x \log (\frac{y}{x}))$.
...
\paragraph{Operation 7 - batched interval insertions:}
As we did in the previous operations, we split the tree into small trees with $\frac{y}{x}$ leaves.	We now construct an augmented 2-3 tree for each interval. This is done in $O(x)$ time, since the breakpoints of the intervals are given in sorted order. We then merge these interval trees into our small trees. The naive way to do this would be to merge each interval tree with one small tree, by either performing one join operation with the small tree, or by performing a split operation and two join operations (in case the interval should be inserted between two leaves of the small tree). This will not work, since we might end up with a few very large trees (e.g. a tree with $\frac{y}{x}+x$ leaves). We need the number of leaves in each of the resulting small trees to be between $\frac{y}{x}$ and $\frac{2y}{x}$, so that the difference of heights of any pair of trees will be at most one, and joining them will be efficient. For this, before merging each interval into a small tree, we check whether the resulting tree will have more than $\frac{2y}{x}$ leaves. If so, we consider two cases: first, if the interval and the leaves of the small tree are disjoint (i.e. we could have merged them with one join operation), we separate the interval to several smaller intervals, one of which is merged with the small tree by a join operation (s.t. the resulting tree has no more than $\frac{2y}{x}$ leaves), and the rest are introduced as new small trees, each of which has no more than $\frac{2y}{x}$ and no less than $\frac{y}{x}$ leaves. In the second case, the interval is to be inserted between two leaves of the small tree, so we split the tree there and similarly merge the interval by performing at most two join operations, and perhaps introducing more small trees. Again, we maintain that the number of leaves in each of the resulting trees is between $\frac{y}{x}$ and $\frac{2y}{x}$. This merging process takes $O(x \log(\frac{y}{x}) + x) = O(x \log (\frac{2y}{x}))$ time.

We are left with at most $\frac{y+x}{\frac{y}{x}} \leq \frac{2y}{\frac{y}{x}} = 2x$ small trees, and we now join all the small trees in order to obtain on large tree. Since the difference of heights between each pair of trees we join is always at most one, the cost of each join operation is constant. Thus, the cost of all the join operations is bounded by: $2x+x+\frac{x}{2} +...+ 2 = O(x)$, and the total cost of the operation is $O(x \log(\frac{2y}{x}))$.

\paragraph{Operation 8 - batched interval deletions:}
Once again we split our tree to small trees. We then delete the relevant intervals from each of the small trees. Each interval is either contained in a single tree, and will be deleted by performing one or two split operations and at most one join operation, or it spans over several trees, in which case it is deleted by two split operations (on the trees where the endpoints of the interval lie), and maybe deleting some trees entirely. Similarly to the batched interval increase operation, this costs $O(x \log(\frac{y}{x}) +x) = O(x \log(\frac{2y}{x}))$ in total.
\begin{theorem}
