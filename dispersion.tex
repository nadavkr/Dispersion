...
We now would like to pre-process the fragments s.t we can later check in constant time for any two nodes in a fragment, $u_1,u_2$ if $d(u,v)\geq\lambda$, and if $d(u_1,u_2) \leq \lambda/2$, for any possible $\lambda$. We achieve this for most of the fragments.

\subsubsection{Pre-Processing}
For each fragment, we would like to implicitly construct matrices of total size $O(blogb \times blogb)$, that are row and column sorted, and contain all pairwise distances in the fragment.
Since each fragment is a tree, we can use the centroid decomposition on it, and at each level of the recursion, compute all the distances to the centroid. In the recursion step we have three subgraphs, and for each of them we have the distances from all %TODO


Now, we can use Frederickson's serching algorithm to eliminate entries of the matrices. %TODO cite. explain?


We finish this phase with an interval $[\lambda_1,\lambda_2)$, s.t the $\lambda^*$ we are searching for in the MMDP, is in this interval. We call fragments that do not contain a pairwise distance the is in the interval \textit{inactive}, and all other fragments \textit{active}.
%TODO how many inactive fragments?
...
Observe that each fragment is comprised of its spine, and the subtrees hanging off of it. Using queries on pairwise distances as described above, we can use our linear feasibility test on the subtrees hanging off the spine, and get a blue and a green node for each of them (this costs linear time, but is only done once in preprocessing).
...
\end{step}
\begin{step}
\textbf{Prune the caterpillar so that the leaves' distances to the bottom is non-decreasing:} %TODO check the direction
...
In query time we will want to process each fragment after we have already processed its hole. Assuming we know what is the closest chosen node in the fragment's hole, we can eliminate a suffix of the caterpillar (because of the monotonicity of the distances of blue nodes from the hole). We can compute in preprocessing, for any possible eliminated suffix, which nodes in the fragment should be chosen, and also which nodes are the blue and green node. %TODO how to do it in blogb
\end{step}
