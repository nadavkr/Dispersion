...
We now present a single step of the algorithm. Denote the root of the subtree by $v$, its left child by $u_1$, and its right child by $u_2$ \footnote{If $v$ only has one child we can simply build the new polyline by querying the previous one for the case that $v$ is in the solution and for the case where it isn't.}. We have two polylines that represent the solutions inside the subtrees rooted at $u_1$ and $u_2$, and we would like to create the polyline for the subtree rooted at $v$.
Assume w.l.o.g. that the number of breakpoints in $u_1$'s polyline (denoted by $p_1$), is smaller or equal to the number of breakpoints in $u_2$'s polyline (denoted by $p_2$). We consider three cases, for each of them construct a polyline, and then merge them into one polyline.
...
We start by shifting $p_1$ to the right by $d(u_1,v)$, since we are now constructing a polyline where the horizontal axis signifies the distance from $v$, not from $u_1$. We then need to increase the value of points in the polyline to account for the weight of the chosen nodes in the subtree rooted at $u_2$. In order to do this we need to query $P_2$ so as to maintain that $d(h_1,h_2) \geq \lambda$ and that $d(h_1,v) \leq d(h_2,v)$. For this, we consider two cases, first, when $2 \cdot d(h_1,u_1) + d(u_1,v) + d(v,u_2) \geq \lambda$ we must query $P_2$ with $d(h_1,u_1)$ and second, when $2 \cdot d(h_1,u_1) + d(u_1,v) + d(v,u_2) < \lambda$ we must query $P_2$ with $\lambda - [d(h_1,u_1) + d(u_1,v) + d(v,u_2)]$.

We go over the breakpoints of $P_1$ and for each of them query $P_2$ as described (considering the value of $d(h_1,u_1)$ at the breakpoints themselves). This may result in a non-monotone polyline, since for some values of $d(h,v)$ the optimal solution isn't one where the closest chosen node is in the subtree rooted at $u_1$. We "smooth" the polyline to be monotone by simply iterating over all the breakpoints of the new polyline and erasing some of them.
\item \emph{The chosen node closest to $v$ inside its subtree, is in $u_2$'s subtree: }
In this case we do not want to query $P_1$ for every breakpoint in $P_2$ (which has more breakpoints than $P_1$).
\end{enumerate}
...
We store these polylines in binary search trees, so we can answer queries in logarithmic time, using predecessor queries. Each node of a binary search will store $d(c)$ as its key, and the size of $P$ when this is the distance of the closest chosen node.
