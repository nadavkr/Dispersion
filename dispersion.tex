...
Recall that in the previous algorithm we stored at the bottom node of every prefix of the fragment
the following information: the number of chosen nodes, the candidate node, and the certain node for this fragment,
assuming that the suffix of the caterpillar below this node is eliminated. Now, we cannot afford to explicitly
compute such information for every node in every iteration of the algorithm (i.e. for each fragment size),
as this might take linear time per iteration. Hence we will only store, for each candidate node in a caterpillar,
