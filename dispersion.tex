...
The above feasibility test is presented in Section~\ref{nloglogn solution}. By appropriately setting up $x$ to balance the preprocessing and the query time, the overall preprocessing time is $O(n\log\log n)$. The bottleneck of this preprocessing are its calls to the vanilla linear-time feasibility test.  
We next improve this gradually to $O(n)$ by constructing a sequence of faster and faster feasibility tests. 
...
so after $O(\log^{*}n)$ iterations we obtain the desired efficient feasibility test.
This gives use a feasibility test with sublinear query and $O(n\log^* n)$ preprocessing (see Section~\ref{sectionlog*}). 

Finally, in Section~\ref{sectionLinear}, we improve the preprocessing from $O(n\log^* n)$ to $O(n)$. 
The main difficulty here is that now we cannot afford to construct each micro-macro decomposition separately. However, this can be overcome by making each decomposition
...
\todo{Add description of the weighted case}
...
\section{$O(n\log^{*}n)$ Algorithm} \label{sectionlog*}
...
\section{Linear time solution for the dispersion optimization problem}\label{sectionLinear}
