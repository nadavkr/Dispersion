...
i.e., on a path. The basic idea of this reduction, is to switch between the edges and vertices, i.e. create a new line, where each of the edges is a node in the original line, and each of the points is an edge originally. We assign the same weights, so now we have weights on the points, and run the $k$-partitioning algorithm.
