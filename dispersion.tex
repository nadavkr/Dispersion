...
developing a feasibility test that requires {\em linear} preprocessing and can then be queried in \emph{sublinear} time. This was the approach of Fredrickson for $k$-partitioning and $k$-center and it is plausible that his framework can be adapted to solve also our problem.
However, we believe that our (slightly different) take on this allows for a simpler and more structured algorithm, and hence might be of
independent interest.
Note that even a sublinear feasibility test is not enough to solve the whole problem in $O(n)$ time, as we cannot afford to represent all the pairwise
distances between the nodes in such complexity. Therefore, the whole algorithm maintains the current interval
interval $[\lambda_1,\lambda_2)$, s.t. $\lambda_1$ is the largest value we have tested and is feasible, and $\lambda_2$ is the smallest value we have tested and found not feasible (which implies that the optimal value, denoted by $\lambda^*$, lies within the interval) and generates,
in every step, an implicit representation of only some of these distances.
...
the matrices that allow us to eliminate many entries of the matrices each time. Then, we are able to prune the tree and repeat.
...
tree is pruned.
Finally, for the weighted case, we present tight $\Theta(n\log n)$ upper and lower bounds for the decision problem, which implies an $\Oh(n\log^2n)$ time solution for the optimization problem. 
...
Finally, in Section~\ref{section:weighted} we consider the weighted case. Our main result is an $O(n\log n)$ feasibility test (for which we also give
a simple tight lower bound). In contrast to the unweighted case, where each subtree had only one candidate, in the weighted case each subtree can
...
and by standard calculation we obtain an $\Oh(n\log^{2}n)$ time feasibility test. To improve on that and obtain an optimal algorithm,
we need to merge in $\Oh(\min(x,y)\log\frac{\max(x,y)}{\min(x,y)})$ time. 
An old result of Brown and Tarjan is that, given AVL trees representing two sorted lists of length $x$ and $y$, an AVL tree representing
their merge can be produced in $\Oh(\min(x,y)\log\frac{\max(x,y)}{\min(x,y)})$ time. This was later generalized by Huddleston and
...
application we need multiple non-standard operations on the lists, and hence applying these results would requires some extra work.
Instead, we present a different (and simpler) approach in Section~\ref{section:data structure} that works with essentially any balanced search
tree with split and join operations. We believe that this might of independent interest due to its simplicity. With this structure in hand,
another standard calculation implies that the total time becomes $O(n\log n)$.
