...
We apply Lemma~\ref{good partition refinement lemma} $\log^{*}n$ times. Denoting by $b_{\ell}$ the value of
the parameter $b$ in the $\ell$-th application, we see that $b_{i+1}=2^{b_{i}}$, so by induction
$b_{\ell}\geq \ell$. The time to construct the $(\ell+1)$-th good partition is $O(n/(b_{\ell})^{4})=O(n/b_{\ell})$,
...
size is bounded by $c^{\ell+1}\cdot B^4$, and so we have at most $O((\ell+1)+\log B)=O(\log B)$ levels in the heavy 
path decompositions. On each level we run Frederickson's search (similarly to what we presented before). We run this 
search in parallel, for all the heavy paths from all these hanging subtrees in all the large fragments. The number of 
nodes in the current level for all the heavy path trees together is at most $n$. We set the parameter $p$ to
$\frac{n}{B^6}$. Aside from linear (in the number of nodes in these subtrees) time processing exclusive of tests
we need to call  our $(\frac{n}{b^4} \times \log b)$ test $O(\log B)$ times for each of the $O(\log B)$ levels.
This takes $O(\frac{n}{b^2} \times \log b) = O(\frac{n}{b})$. Summing over all $\log^{*}n$ iterations, this
is $O(\sum_{\ell}\frac{n}{b_\ell}) = O(n)$.
...
the parameter $p$ to $\frac{n}{B^{5}}$ in the Frederickson's search, and thus considering all the levels there
might be up to $\log B\times\frac{n}{B^{6}}\leq \frac{n}{B^{5}}$ large fragments in which we cannot fully reduce
all the hanging  subtrees. The total number of nodes in all such active fragments is  at most
$\frac{n}{B^5} \times O(B^4) = O(\frac{n}{B})$. We cannot amortize the time spent on active fragments by
reducing them, but we can afford to separately pay $O(\frac{n}{B})$ per iteration for that.

Having processed the hanging subtrees, we now have to to deal with active small fragments that are on the spine
of the large fragment. In the previous iteration we had $O(\frac{n}{b^5})$ active fragments, which is of course
...
Frederickson's search with $p$ set to $\frac{n}{B^6}$). In each fragment we have $O(\log b)$ levels of the heavy
...
We now have $O(\frac{n}{B^5})$ active small fragments in total. We declare each large fragment that contains
...
caterpillar, starting from the topmost candidate node in the next small caterpillar. 
...
node. Assuming such representation, we go over the large caterpillar from bottom to top while
maintaining a stack with the already processed small caterpillar that still contain some candidate nodes.
...
accessing it top candidate node. If its distance from the root is too small, we remove it from the
doubly-linked list and repeat. Now the distances of the remaining candidates from the root are
...
Now we need to eliminate all internal "collisions" inside the caterpillar (i.e. remove candidate nodes that
...
candidates below and above its small caterpillar. Because 

candidate nodes that form
