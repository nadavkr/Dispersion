...
\item Batched value predecessor - Given a list of key-value pairs, $\{(k_i,v_i)\}$, find for each $k_i$, the maximal key $k_{i}'$, s.t. $k_{i}' \leq k_i$ and the value of the polyline at $k_{i}'$ is larger than $v_i$.
...
In order to implement these operations efficiently, we represent the polylines by storing their breakpoints in augmented balanced search trees. Each node of the trees will store a key-value pair. The breakpoints of the polyline will be stored at the leaves of the tree, and we will maintain the following property: the key of each leaf will be the sum of the keys of all its ancestors, and its value will be the sum of the values of all its ancestors.

Let us describe the implementation of each operation, by using the augmented balanced search trees. Operation 1 can simply be implemented by adding the required distance to the key at the root of the tree. Operations 2 and 3 can use standard split and merge procedures for balanced search trees. Operation 4 requires saving an extra bit in the root of the tree, thus allowing to negate all the keys in the tree by switching it. After the negation, the mirroring operation will add $\lambda$ to all the keys in the tree (by adding to the root). Operation 5 can be done by a predecessor query, and returning the value at the previous breakpoint.

Operation 6 - batched interval increase: we iterate over the intervals and for each interval, we traverse the tree from the starting point of the interval, through the LCA, and to the ending point of the interval. We store in each node of the tree the maximum of its subree, so we know when we reach the LCA. On the way up to the LCA we increase the key of every right child which is not on the path, and on the way down from the LCA we increase the key of every left child that is not in the path. 

Operation 7 - batched value predecessor: We store in each node the minimum value in its subtree. Each value predecessor query will be answered by going up the tree until the minimum in the current node's subtree is smaller or equal to the value of the starting point, and then go down and search for the value.

Operation 8 - Batched interval deletions:

Operation 9 - Batched interval insertions:

Operations 1-5 will be done only a constant number of times per step. 
