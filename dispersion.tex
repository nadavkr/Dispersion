...
all the hanging subtrees. The total number of nodes in all such large fragments is at most
$\frac{n}{B^5} \times O(B^4) = O(\frac{n}{B})$. We cannot amortize the time spent on processing the hanging
subtrees there by simply saying that each such hanging subtree is reduced to at most
two nodes, but we can afford to separately pay $O(\frac{n}{B})$ per iteration for this processing.
