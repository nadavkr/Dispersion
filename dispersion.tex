...
Our linear-time preprocessing is based on the $O(n \log ^*n)$ preprocessing presented in the previous section. In order to achieve linear time
...
\end{lemma}
\begin{proof}
Define $T'$ as the tree obtained by collapsing each small fragment (of the given partition) to a single node. Apply 
...
which can be bounded by: $$ O(\sum_{\ell} \frac{n}{b_{\ell}}) = O(\sum_{\ell} \frac{n}{2^{\ell}}) = O(n).$$
After every application, we construct the corresponding $O(\frac{n}{(b_{\ell+1})^{4}}\cdot \log b_{\ell+1})$ time
test given the previous $O(\frac{n}{(b_{\ell})^{4}}\cdot \log b_{\ell})$ time test and a good partition of the tree
...
\end{center}
\caption{A large fragment containing a number of smaller fragments. The small fragments on the spine are gray, and the subtrees hanging off of the spine are black. We start by reducing the hanging subtrees to just two nodes, then handle the small active fragments on the spine, and finally reduce the entire large fragment to one caterpillar and process it. \label{figure of small fragments inside a large fragment}}
\end{figure}


We start by reducing the hanging subtrees to at most two nodes, one candidate and one certain (Lemma~\ref{lemma1}), then we handle the small active fragments on the spine (Lemma~\ref{lemma2}), and finally we reduce the entire large fragment to one caterpillar and process it (Lemma~\ref{lemma3}). 

\begin{lemma}\label{lemma1}
	The hanging subtrees of all large fragments can be reduced to at most two nodes (one candidate and one certain) in total $O(n)$ time. 
\end{lemma}
\begin{proof}
We 
...
the parameter $p$ to $\frac{n}{B^{10}}$, so the number of calls to the $(\frac{n}{b^4} \cdot \log b)$ time test
...
$\frac{n}{B^9} \cdot O(B^5) = O(\frac{n}{B^{4}})$. We cannot amortize the time spent on processing the hanging
...
\begin{lemma}\label{lemma2}
	The small active fragments on the spines of all large fragments can be reduced to catterpillars in total $O(n)$ time. 
\end{lemma}
\begin{proof}
...
the following properties.  Once we have these properties, we will
...
This takes $O(\log (b^{5}))$ time per small caterpillar, so $O(\frac{n}{b^{4}}\cdot \log b)=O(\frac{n}{b})$ in total.
...
and its predecessor). During the preprocessing, we need to merge multiple stable infixes
