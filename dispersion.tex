...
In the previous section, we partitioned the input tree into fragments independently for each fragment size. This resulted in $\log^{*}n$ partitions, each preprocessed in $O(n)$ time. In order to obtain overall $O(n)$ time, we first introduce a dependency between the partitions: The fragments of the $(\ell+1)$-th partition are composed of fragments of the $\ell$-th partition. To this end, we relax the definition of a {\em good partition} so that, in a partition into $n/b$ fragments, a fragment can consist of more than $O(b)$ nodes. The partitioning is simple and is given in Subsection~\ref{section:partioioning}.  
The main challenge of this section is to extend the feasibility tests on the fragments of the $\ell$-th partition (small fragments) to feasibility tests on the fragments of the $(\ell+1)$-th partition (large fragments), such that all iterations take overall $O(n)$ time.
...
\noindent In Subsection~\ref{section:lemma1} we show how to reduce the hanging subtrees to at most two nodes, one candidate and one certain. Then, in Subsection~\ref{section:lemma2} we handle the small active fragments on the spine, and finally in Subsection~\ref{section:lemma3} we reduce the entire large fragment into one caterpillar and process it. 
...
In the rest of this section, we will show how to obtain in amortized $O(\frac{n}{b_\ell})$ time a feasibility test with $O(\frac{n}{(b_{\ell+1})^{4}}\log (b_{\ell+1}))$ query-time given a feasibility test with $O(\frac{n}{(b_{\ell})^{4}}\log (b_{\ell}))$  query-time.
% 
...
\subsection{Reducing the hanging subtrees}\label{section:lemma1}
...
\subsection{Reducing the small active fragments}\label{section:lemma2}
...
\subsection{Reducing the large inactive fragments}\label{section:lemma3}
