...
\subsection{Reducing each hanging subtree into two nodes}\label{section:lemma1}
...
\subsection{Reducing each small active fragment into a caterpillar}\label{section:lemma2}
Having processed the hanging subtrees, we now deal with the active small fragments the spine of the large fragment. 
...
We now have $O(\frac{n}{B^9})$ active small fragments in total. We declare each large fragment that contains a small active fragment as active. 

\subsection{Pruning the resulting large catterpillar}\label{section:lemma3}
So far, each inactive large fragment was reduced to one large caterpillar which is the concatenation of caterpillars from the smaller fragments. We want
to process this caterpillar (as we have processed the inactive fragments before) in order to ensure
...
\subsection{Preprocess the resulting large caterpillar for every eliminated suffix}\label{section:lemma4}
