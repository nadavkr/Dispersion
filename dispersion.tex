...
%For the decision version, where the goal is to find a set of nodes of total weight at least $k$ such that any two nodes are at distance at least $\lambda$, we present tight $\Theta(n\log n)$ upper and lower bounds. For the optimization version, where we wish to maximize  $\lambda$, we present an $\Oh(n\log^2n)$ upper bound improving the previous $\Oh(n\log^4 n)$ time solution. 
\end{abstract}
...
%distance is multiplied by the corresponding weight. The currently fastest solution is the $\Oh(n\log^2n)$ time algorithm of Megiddo and Tamir %cite Cole and \cite{Megiddo1983}.
...
%Then, we move onto the weighted version, where the goal is to select nodes with total weight at least $k$. There, the best previously known solution requires $\Oh(n\log^4 n)$ time \cite{Bhattacharya1991}. We present a faster $\Oh(n\log^2 n)$ time algorithm. We also show that the decision version, where the goal is to check if there exists a set of nodes total weight $k$ such that any two nodes are at distance at least $\lambda$, requires $\Omega(n\log n)$ time in the algebraic decision tree model. We also present a matching $\Oh(n\log n)$ time algorithm, which is then used to construct our $\Oh(n\log^2n)$ time solution for the weighted dispersion problem using the standard parametric search paradigm of Fredrickson. 
...
Before presenting our optimal linear algorithm for the dispersion optimization problem, we first present $O(n\log \log n)$ and $O(n\log ^*n)$ algorithms. Each of these algorithms builds on the previous, and uses some new ideas to improve the complexity. All three algorithms are based on the sublinear algorithm we present for the feasibility test in Section \ref{sublinear f.t.}. This algorithm uses the simple linear algorithm presented in Section \ref{linear F.T.}, which is similar to the one presented in \cite{Bhattacharya1991}. We use a new technique for partitioning trees, which separates the tree to fragments with at most two boundary nodes that connect them to the rest of the tree. This allows us to preprocess the resulting fragments in linear time, s.t. we are able to run feasibility tests in sublinear time. This partitioning is similar to the micro-macro decomposition, but our partitioning scheme takes only linear time overall for partitioning the tree into fragments of increasing sizes, which allows us to use a bootstrapping technique in linear time. %TODO add citation for micro-macro

We use the sublinear feasibility test we create, and build upon a parametric search method presented by Frederickson, and used by him to solve several problems (e.g. the max-min tree $k$-partitioning \cite{Frederickson1991}), to solve the dispersion problem in $O(n)$ time. At a glance, it seems that the same technique used to provide an optimal algorithm for $k$-partitioning in \cite{Frederickson1991} might also work for the dispersion problem. Indeed, in the one dimensional case it does work and one can easily reduce the dispersion problem for a line to the $k$-partitioning problem for a line. Trying to implement this for the more difficult case of a tree fails. The reason for this is that in the $k$-partitioning problem, there is no tie between nodes in different segments of a partition (i.e. a solution for the partitioning problem), and this significantly simplifies the problem. We overcome this, by using some new techniques and tricks, enhancing the useful searching method by Frederickson. We use a more elaborate way of trimming the input tree, than what was previously presents. We reduce parts of our tree to caterpillar graphs, which are easier to handle and process.
