...
to binary searching for how long should be the eliminated prefix. In the $i$-th step we
...
such that $u$ might be accessed in the query time. Recall that in the query time we specify
...
of the large caterpillar. But distance of $v$ from the spine is less than $\frac{\lambda^{*}}{2}$
and then it follows that the distance of $u_{i}$ to the hole of its small caterpillar is also less
...
In the query time, we first use the balanced search tree storing the unstable suffix to locate the first node
...
Recall that, for each candidate node there, its jump points leads to the closest candidate node above
...
an interesting prefix or suffix (or being a new node) and call them relevant. In particular,
...
has two credits. When a node becomes a part of a stable infix we treat it a small fresh
