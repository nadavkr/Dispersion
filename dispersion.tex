...
\end{center}
\caption{The polyline represents the weight of the solution, $P$, as a function of the distance of closest chosen node in the subtree. $v$ is the root of the subtree, and $h$ is the chosen node closest to $v$ in its subtree. The weight of $P$ only decreases at certain points, which are the breakpoints of the polyline.
\label{figure of a polyline}}
\end{figure}

We now present a single step of the algorithm. Denote the root of the subtree by $v$, its left child by $u_1$, and its right child by $u_2$ \footnote{If $v$ only has one child we can simply build the new polyline by querying the previous one for the case that $v$ is in the solution and for the case that it isn't.}. We have two polylines that represent the solutions inside the subtrees rooted at $u_1$ and $u_2$, and we would like to create the polyline for the subtree rooted at $v$.
Assume w.l.o.g. that the number of breakpoints in $u_1$'s polyline (denoted by $p_1$), is smaller or equal to the number of breakpoints in $u_2$'s polyline (denoted by $p_2$). We postpone the discussion of the data structure we use to store the polylines for now. 
...
Let us consider the case where $v$ is in the solution. In this case we simply query $p_1$ and $p_2$ for their values with distances of the closest node being $\lambda - d(v,u_1)$ and $\lambda - d(v,u_2)$ (if one of these values is negative, we take zero instead of it), add them together with the weight of $v$, and get the weight of the solution for $v$'s subtree in case $v$ is in the solution. This will be the value of the new polyline for $d(h,v)$ between zero and the first breakpoint.

We now construct the rest of the polyline. Notice that we need to maintain that $d(h1,h2) \geq \lambda$. First, we need to shift $p_1$ and $p_2$ right by $d(v,u_1)$ and $d(v,u_2)$ respectively, as we now measure the weight of the solution as a function of the distance of the closest chosen node to $v$, and not $u_1$ or $u_2$. We proceed in two steps:

\begin{enumerate}
\item We construct the second part of the polyline, where $d(h,v) \geq \frac{\lambda}{2}$. In this case we query both polylines with the same value, as $d(h_1,v) \geq \frac{\lambda}{2}$ and $d(h_2,v) \geq \frac{\lambda}{2}$ implies that $d(h_1,h_2) \geq \lambda$. So, we go over all the breakpoints of $p_1$ (the smaller of the polylines) for values of $d(h_1,v)$ that are greater or equal to $\frac{\lambda}{2}$ and insert each of them to $p_2$, with the value ($W(P)$) increased by the value of $p_2$ at the same point (i.e. store the weight of the solution for the entire subtree of $v$, by adding the weight gained in the subtrees rooted at $u_1$ and $u_2$). This may result in a non-monotone polyline, which we smoothen by iterating over all the breakpoints in the new polyline and removing the ones who are followed by breakpoints with higher values of $W(P)$.
\item We now need to handle values of $d(h,v)$ which are smaller than $\frac{\lambda}{2}$. This is done by iterating over all the breakpoints of $p_1$, while querying $p_2$ with $\lambda - d(h_1,v)$. This basically means that we go over $p_1$ from left to right, while going over $p_2$ from right to left. Notice that as we do this the distance of one of the nodes ($h_1$ and $h_2$) is at most $\frac{\lambda}{2}$. Similarly to before, we increase $W(P)$ at the relevant points, and once more smoothen the polyline.
...
\item Query predecessor.
\item increase the value of $W(P)$ for some breakpoint. 
\end{enumerate}

In order to implement these queries efficiently, we store these polylines in standard balanced binary search trees (such as Red Black Trees), so we can answer queries in logarithmic time. Each node of the trees will store $d(h,v)$ as its key, and $W(P)$ for this key.

Each such step, where we merge two polyline will cost $m \log n$, where $m$ is the number of breakpoints in $p_1$ and $n$ is the number of breakpoints in $p_2$ (so $m \leq n$). This sums up to $n \log^2 n$ overall. Removing breakpoint in order to smoothen the polyline costs linear time overall, since any breakpoint that is removed, will not be processed in the future.
