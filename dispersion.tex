...
The algorithm we present computes such a polyline for the subtrees rooted at all the nodes of the tree. As we mentioned, we will compute this bottom-up, and so in each step of the algorithm we compute the polyline for a subtree rooted at some node $v$, after we have already computed the polylines for the subtrees rooted at his children. When constructing a polyline, we need to consider several cases, as the closest chosen node in the subtree could be the root, or it could be in the subtrees of one of its children. This means that for any key $d(h,v)$ of the polyline, the value in that point could represent a solution $P$ in which $h$ is in the subtree rooted $v$'s left child, or in the subtree rooted $v$'s right child, or it could be $v$ itself.\footnote{For the same reasoning as in the unweighted case, we assume w.l.o.g. that the input tree is binary.}
...
We now construct the rest of the polyline. Notice that we need to maintain that $d(h_1,h_2) \geq \lambda$ (where $h_1$ is the closest chosen node in $u_1$'s subtree and $h_2$ is the closest chosen node in $u_2$'s subtree). First, we need to shift $p_1$ and $p_2$ right by $d(v,u_1)$ and $d(v,u_2)$ respectively. We then proceed in two steps:
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=.7\textwidth, height=0.3\textheight]{new_polyline_second_half}
\end{center}
...
\item We now need to handle the case where $d(h,v) < \frac{\lambda}{2}$. For this, we need to consider two possible cases: first, the case where $d(h_1,v) < d(h_2,v)$ (i.e. the closest chosen node in $v$'s subtree is inside $u_1$'s subtree), and second, the case where $d(h_1,v) > d(h_2,v)$. For each of the two cases we will construct the first half of the polyline (where $0 \leq d(h,v) \leq \frac{\lambda}{2}$), and then we take the maximum of the two polylines at every point.

Let us start with the first case. We need to consider possibilities where $d(h_1,v) < \frac{\lambda}{2}$, which implies that $d(h_1,v) > \frac{\lambda}{2}$, and furthermore $d(h_2,v) \geq \lambda - d(h_1,v)$. In this case the naive algorithm would iterate over $p_1$ from zero to $\frac{\lambda}{2}$ while iterating over $p_2$ from $\lambda$ to $\frac{\lambda}{2}$ (i.e. from right to left), and sum the values of the two polylines, but again, we cannot afford to iterate over the breakpoints of $p_2$. We start by splitting $p_1$ at $\frac{\lambda}{2}$ and taking the first half (denote it by $p_1'$), then we split $p_2$ (also at $\frac{\lambda}{2}$), take the second half, and "mirror" it (so that we can iterate over the resulting polyline, $p_2'$, from left to right as usual). We would now like to obtain the sum of the values of the two resulting polylines at each each point. This is done similarly to before, the breakpoints of $p_2'$ induce intervals of $p_1'$, and we lift up each such interval. Notice that since $p_1'$ is a monotone decreasing polyine, and $p_2'$ is a monotone increasing polyline, the resulting polyline may not be monotone. This can be solved by first observing that we could look at only the last breakpoint of $p_2'$ in each interval induced by the breakpoints of $p_1'$ (since the previous breakpoints can surely be ignored, as they represent lighter solutions, where $h$ is at the same distance from the root). In order to do this, we must find the last breakpoint in each interval of $p_2'$ induced by the breakpoints of $p_1'$ (this is done by a predecessor query on $p_2'$), and delete all other breakpoints of $p_2'$ in the interval. We then increase the value at the remaining breakpoint by the value of $p_1'$. We then need to make sure the resulting polyline is monotone, by iterating it from $\frac{\lambda}{2}$ to zero, and deleting unnecessary breakpoints. Notice that we can do that, since now the number of breakpoints in the resulting polyline will be at most twice the number of breakpoints in $p_1$ (since for every interval between breakpoints of $p_1'$ we have at most one breakpoint from $p_2'$).

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.4]{polyline_first_half_construction_case1}
\end{center}
\caption{The first step of constructing the first half of the polyline. $p_1'$ (in blue) is the first half of $p_1$, $p_2'$ (in red) is the second half of $p_2$, mirrored. The black polyline is the result of summing the value of the two polylines for each point, and so it not a monotone polyline.
\label{figure of constructing the second half of the polyline}}
\end{figure}


We now consider the second case, where $d(h_1,v) < d(h_2,v)$ (and $d(h,v) < \frac{\lambda}{2}$). This time we would have liked to iterate over the first half of $p_2$ while iterating over the second half of $p_1$ backwards. We split both polylines at $\frac{\lambda}{2}$, take the first half of $p_2$ and the second half of $p_1$. We then increase the intervals of $p_2$ induced by breakpoints of $p_1$ by the appropriate values, as before. Again, the resulting polyline may be non-monotone, but this time we cannot solve the problem as in the previous case, since the number of breakpoints can be the number of breakpoints of $p_1$ plus the number of breakpoints of $p_2$ in the worst case. In order to make the polyline monotone, we need to delete breakpoints in some intervals of the polyline. These intervals can be found by running \emph{value predecessor} queries. For each key of a breakpoint of $p_1$ we find the value of the same key in the new polyline. Denote this value by $y$. We then find the last point (i.e. the point with maximal key) in the new polyline s.t. its value is greater or equal $y$ (i.e. a value predecessor query). After finding all such intervals (each of which is defined by a breakpoint of $p_1$, and the point we found), we delete all the breakpoints inside these intervals. This ensures that the resulting polyline is monotone. %The resulting polyline's number of breakpoints is linear in the number of breakpoints of $p_2$.
...
We now need to build one polyline for the first half of the polyline, taking into account both cases. This is done by taking the maximum of the values of the polylines we just built (denoted by $p_a$ and $p_b$), for any point (s.t. $0 \leq d(h,v) \leq \frac{\lambda}{2}$). We iterate over the smaller of the two resulting polylines (assume w.l.o.g. it is $p_a$), and for every pair of consecutive breakpoints, we check which polyline is higher at the points (i.e. for which polyline $W(P)$ is larger). If the answer for both points is the same, we carry on. Otherwise, we binary search over the breakpoints of $p_b$ in this interval (defined by the two consecutive breakpoints of $p_a$), for the breakpoint at which the two polylines intersect. In the end of this iteration we know which polyline gives us the best solution for every point between zero and $\frac{\lambda}{2}$, and where are the intersection points where this changes. We can now build the new polyline by doing insertions and deletions in $p_a$, according to the intersection points. For every interval of $p_a$ defined by a pair of consecutive intersection points, we check if the value of $p_b$ is larger than the value of $p_a$ in the interval, and if so, delete all the breakpoints of $p_a$ in the interval, and insert the breakpoints in this interval from $p_b$. This can be viewed as deleting all breakpoints in some list of intervals of $p_a$, and then a batched insertion of some breakpoints, between breakpoints of $p_a$. Since the two polylines are monotone decreasing, the intersection points can only be at the breakpoints of $p_a$, and at most at one breakpoint of $p_b$ between two consecutive breakpoints of $p_a$, and thus the number of intersection points is linear in the number of breakpoints of $p_a$ (which is linear in the number of breakpoints of $p_1$). In total, the number of interval deletions and insertions we do is linear in the number of breakpoints of $p_1$. Merging this polyline with the polyline we got for $d(h,v) \geq \frac{\lambda}{2}$, will give us the polyline representing the solution $P$, for every possible distance of the closest chosen node in $v$'s subtree.
...
\item Query the weight of $P$ for a certain key $d(h,v)$.
\item Batched interval increase - Given a list of disjoint intervals of the polyline, and a number for each interval, increase the value of the polyline in each interval by the appropriate number.
\item Find the last breakpoint in some interval of the polyline.
...
In order to implement these operations efficiently, we store the polylines in augmented balanced search trees. Each node of the trees will store $d(h,v)$ as its key, and $W(P)$ for this key. The breakpoints will be represented at the leaves of the tree, and the key and value of each leaf will be the sum of all its ancestors.

Operations 1-4 will be done only a constant number of times per step. The number of times operations 5-9 will be performed is linear in the number of breakpoints of $p_1$ (in each step), and they will be performed on $p_2$. The augmented balanced search trees we use here can answer such queries in logarithmic time. Querying the value at some point of the polyline can be done by a predecessor query. Shifting the polyline can be done by increasing the key at the root of the augmented tree. Increasing the value for some interval can be done by adding to the value in roots of subtrees hanging off of the paths from the endpoints of the interval to their LCA. The negation needed for the "mirror" operation can be done by saving an extra bit in all nodes of the tree. Finding the last breakpoint in some interval of a polyline can also be done by a predecessor query.
Batched interval deletions will be done by going up to the LCA of the endpoints of each interval, and deleting its entire subtree.
Batched interval insertions will be done just as a single insertion.

Each such step, where we merge two polyline will cost $x \log y$, where $x$ is the number of breakpoints in $p_1$ and $y$ is the number of breakpoints in $p_2$ (so $x \leq y$). This sums up to $O(n \log^2 n)$ overall in the worst case.


\subsection{An \boldmath$O(n \log n)$ Algorithm for the weighted feasibility test}
We now present an optimal $O(n \log n)$ time algorithm for the weighted feasibility test. We do this by building upon the previous algorithm, and reducing the running time of the heavier operations we run on the polylines. Previously, we had a running time of $O(x \log y)$ for these operations, and we will now show how we can reduce this to $O(x \log (\frac{y}{x}))$ time per operation, which will induce a running time of $O(n \log n)$ for the entire algorithm. We achieve this running time by using a result by Huddelston and Melhorn. In %TODO cite
the writers prove some properties of Finger Search Trees which we use. The main idea here is to analyse the complexity of operations more carefully, by looking at the performance of the basic operations we do on our balanced trees (i.e. search, insert and delete) in the context of the interface we defined, and what we do in each step of the algorithm. We will show that since these basic operations are not random, the amortized cost of each operation is less than logarithmic.


\begin{lemma}
For any B tree with $n$ nodes, and a subset of the nodes of the tree of size $m$, s.t. no node in the subset is an ancestor of another node in the subset, the sum of the heights of the subset nodes is $m \log (\frac{n}{m})$.
\end{lemma}
\begin{proof}
Since the tree is balanced, the height of each of the nodes in the subset, is at most $\log n - d_i$ (where $d_i$ is the depth of the node). So, we need to show that $\Sigma{i=1}{m} d_i \geq m \log m$. Consider the tree that is obtained by removing the subtree of each node in the subset. This tree has $m$ leaves (the nodes of the subset), and since every non-leaf node has a degree of at least two, the number of inner nodes is at most $m/2$.
\end{proof}

\begin{lemma}
Operation 5 - Batched interval increase, is performed in $O(x \log (\frac{y}{x}))$ time in each step.
\end{lemma}
\begin{proof}
It is easy to see that the cost of each single interval increase is linear in the height of the LCA of the endpoints of the interval. Thus, we need to prove that the sum of the heights of these LCA nodes, is at most $x \log (\frac{y}{x})$. Since the intervals are disjoint, we know that it is not possible for one of these nodes to be an ancestor of another of these nodes.
\end{proof}
