...
\item Else:
In this case we need to consider the suffixes of the current fragment. We compute the suffix of the fragment that cannot be taken. We can do this by binary searching for the first node of the fragment that is at distance at least $\lambda$ from the current candidate node, $u$. We have pre-computed all we need to know once we have the relevant prefix.
\end{enumerate}
