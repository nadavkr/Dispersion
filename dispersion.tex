...
closes facility. Even this simplest version is already NP-hard (\cite{Vazirani2003}). However, a simple greedy 2-approximation algorithm is known (cite Gonzalez, Teofilo F. (1985), "Clustering to
minimize the maximum intercluster distance", Theoretical Computer
Science, 38, Elsevier Science B.V., pp. 293–306,
doi:10.1016/0304-3975(85)90224-5). The situation for different
...
https://people.orie.cornell.edu/shmoys/approx00.ps and the references
therein.
...
graphs. The simplest yet still nontrivial such a family are trees on
...
O(n) time and space algorithm given by Frederickson (cite WADS) that
solves all but one of them. This was an improvement on the previous
$O(n\log n)$ time algorithm (cite FJ1), which in turn was an improvement
on the $O(n\log^2n)$ time algorithm (cite MTZC). Also weighted version
...
Megiddo \& Tamir). Among other facility location problems on trees there
is the max-min tree k-partitioning (cite PeS) and the min-max tree
k-partitioning (cite PBS), both solved by Frederickson in $O(n)$ time
(cite F2) using a clever approach based on the parametric search that
...
(cite) and takes $O(n\log n)$ time, improving on an earlier $O(n^2)$
solution (cite Wang and Kao, but please look inside first). Using the
...
solution requires $O(n\log^4 n)$ time (cite
http://dl.acm.org/citation.cfm?id=689402). We present a significantly
...
For any subtree of $T$ rooted at node $r$, let $u$ be the most distant active vertex from $r$ in the subtree. Assume $\exists P \subseteq V$ s.t. $f(P)\geq\lambda$, then if $d(u,r)\ge\frac{\lambda}{2}$, $\exists P' \subseteq V$ s.t. $f(P')\geq\lambda$ and $u \in P'$.
\end{lemma}
\begin{proof}
Assume for contradiction that there is no such $P'$. Denote the closest vertex to $u$ in $P$ by $u'$.
We assume that we cannot replace $u'$ with $u$ and still get a valid solution. This means that there is a vertex $x$, s.t. $x\in P$ and $d(u,x)<\lambda$. 
We know that:
...
\corollary{\label{corol4} $d(x,u)<\lambda$ (by definition)}
...
$d(u,u')=d(u,r)+d(r,u')<\lambda$ (due to corollary \ref{corol2}) $\Rightarrow d(r,u')<\frac{\lambda}{2}$ (due to corollary \ref{corol1}), and similarly $d(r,x)<\frac{\lambda}{2} \Rightarrow d(x,u')<\lambda$ in contradiction to corollary \ref{corol3}. 
...
for this search, we use theorem 2.1 in \cite{FredericksonTreePartitioning}. In our case the theorem implies that we can use $O(logb)$ feasibility tests and discard all but $logb$ elements of the matrices. This means that we have at most $logb$ active fragments (out of all $n/b$ fragments). \\ %TODO citation. can we do log(blogb) tests to eliminate all elements?
...
\end{case}
Each feasibility test will cost us $O(logb)$ for each inactive fragment, and $O(b)$ for each active one. In total $O(\frac{n}{b} \times logb) + O(blogb) = O(\frac{nloglogn}{log^2n})$.
