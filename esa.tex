...
Recall that $\lambda^*$ is the distance between an unknown pair of vertices in the tree. The first goal of the preprocessing step is to eliminate many possible pairwise distances, so that $\lambda^*$ is in an interval $[\lambda_1,\lambda_2)$. We want this interval to be sufficiently small, so that for (almost) every fragment, no distance between any pair of nodes in the fragment is in  $[\lambda_1,\lambda_2)$. We call such fragments {\emph inactive}. Roughly speaking, inactive fragments are not sensitive to changes in $\lambda$ in the range  $[\lambda_1,\lambda_2)$. In other words, handling an inactive fragment during a feasibility test is the same for any $\lambda$ in the interval $[\lambda_1,\lambda_2)$. The second goal of the preprocessing step is to precompute, for each inactive fragment, information that only depends on the interval $[\lambda_1,\lambda_2)$ so that we will be able to process the fragment in $O(\log b)$ time during any feasibility test with $\lambda \in [\lambda_1,\lambda_2)$.
...
(a matrix is called sorted if every row is monotone and every column is monotone), s.t. any pairwise distance (and any pairwise distance multiplied by 2) inside the fragment is an entry in some matrix. This is done using the standard centroid 
