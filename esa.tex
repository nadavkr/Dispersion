...
\section{A Linear Time Feasibility Test}\label{linear F.T.}
...
In each step we are given a node $r$, its children nodes $r_{1},r_{2},\ldots,r_{k}$, and, for each child $r_{i}$, a maximal valid
solution $P_{i}$ for the feasibility test on $T_{r_{i}}$ together with the corresponding candidate and certain nodes.
We obtain a maximal valid solution $P$ for the feasibility test on $T_{r}$ as follows:
\begin{enumerate}
\item Take all the nodes in $P_{1},\ldots,P_{k}$, except for the candidate nodes.
\item Take all candidate nodes $u$ s.t. $d(u,r) \geq \frac{\lambda}{2}$ (i.e., they are certain w.r.t. $r$).
\item If it exists, take $u'$, the candidate node farthest from $r$ s.t. $d(u',r) < \frac{\lambda}{2}$ and $d(u',x)\geq \lambda$, where $x$ is 
the already taken node nearest to $u'$.\label{linear time step 3}
\item Take $r$ if $d(r,x)\geq\lambda$, where $x$ is the already taken node nearest to $r$.
\end{enumerate}
...
\section{A Sublinear Time Feasibility Test with \texorpdfstring{\boldmath$O(n\log\log n)$}{O(nloglogn)} Preprocessing}\label{sublinear f.t.}

To accelerate the linear-time feasibility test described in Section~\ref{linear F.T.}, we will partition the tree into fragments.
...
For an inactive fragment, we can now check if the distance between two nodes inside is at least $\lambda$ or $\lambda/2$
by comparing it to $\lambda_{1}$.
...
\section{A Sublinear Time Feasibility Test with \texorpdfstring{\boldmath$O(n)$}{O(n)} Preprocessing}\label{sectionLinear}
...
produces $P$, a subset of the vertices of the subtree rooted at $r$, s.t. $f(P)\geq\lambda$ and $|P|$ is maximal. If there is a tie, we
additionally maximize the distance of the nearest chosen node to $r$. The proof is by induction.

Consider a node $r$ of the tree and and assume that the invariant holds for all of its children $r_{1},r_{2},\ldots,r_{k}$.
