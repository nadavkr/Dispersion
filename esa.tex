...
\end{itemize}
In other words, we wish to select $k$ nodes that are as spread-apart as possible. The dispersion problem can be seen as a generalization of the classical maximum independent set problem (that can be solved by binary searching for the largest value of $k$ for which the minimum distance is at least 2).  
%
...
The optimization problem can be solved by repeatedly querying a {\em feasibility test} that solves the search problem. 
\begin{itemize}
