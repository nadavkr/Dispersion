...
this $\ell$ depends only on the constant $\alpha$).  Denoting by $b=b_{\ell}$ and $B=b_{\ell+1}$  we conclude that every large
of the $(\ell+1)$-th partition is of size $O(B^5)$ and is composed of small fragments of the $\ell$-th partition (each of size $O(b^5)$). 
