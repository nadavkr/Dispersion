...
solution $P_{i}$ for the feasibility test on $T_{r_{i}}$, the candidate node (w.r.t $r_i$), and the certain node (w.r.t $r_i$). We obtain a maximal valid solution $P$ for the feasibility test on $T_{r}$ as follows:
\begin{enumerate}
\item Take all the vertices in $P_{1},\ldots,P_{\ell}$, except for the candidate vertices.
...
with maximal cardinality, the distance between $r$ and the node closest to it which is in the subset, is maximized in $P$. The proof is by induction.
