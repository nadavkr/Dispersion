...
%
It turns out that the $k$-dispersion and the $k$-partitioning problems are actually equivalent in the one-dimensional case (i.e., when the tree is a path). The reduction simply creates a new path whose edges correspond to nodes in the original path and whose nodes correspond to edges in the original path. However, such equivalence does not apply to general trees, on which $k$-dispersion seems more difficult than $k$-partitioning. In particular, until the present work, no linear time solution for $k$-dispersion was known. The fastest solution was the $O(n \log n)$ solution given in the late 90's by Bhattacharya and Houle \cite{Bhattacharya1991}, who also considered the weighted version (where nodes also have weights), for which they gave an $\Oh(n\log^4 n)$ time solution \cite{Bhattacharya1999}.
...
\item  {\em The Dispersion Decision Problem (feasibility test).}  Given a tree with non-negative edge lengths, a number $k$, and a number $\lambda$, find a subset $P$ of nodes of size $k$ such that  $f(P)\geq\lambda$, or declare that no such subset exists. 
\end{itemize}

Bhattacharya and Houle~\cite{Bhattacharya1991} present a linear-time feasibility test, and use a result by Fredrickson \cite{Frederickson1983} that enables binary searching over all possible values of $\lambda$ (i.e., all pairwise distances in the tree). In other words, a feasibility test with a running time $\tau$ implies an $O(n \log n + \tau \cdot \log n)$ time algorithm for the dispersion optimization problem. We present a simplified version of their linear time
feasibility test in Section~\ref{linear F.T.}.
...
$P$ of nodes of total weight at least $W$ s.t. $f(P)$ is maximized. \cite{Bhattacharya1999} present an efficient feasibility test for this generalization, that by the same reasoning above solves the weighted optimization problem with a logarithmic overhead.
 
\medskip \noindent {\bf Our solution for the unweighted dispersion problem.}
We are able to decrease the total time to $O(n)$ by
developing a feasibility test that requires {\em linear} time preprocessing and can then be queried in \emph{sublinear} time. This was Fredrickson's approach for $k$-partitioning and $k$-center, and it is plausible that his framework can be adapted to solve our problem as well.
...
% 
The first step of the preprocessing computes values $\lambda_1$ and  $\lambda_2$ such that (1) there is no solution to the decision problem on $T$ for any $\lambda \geq \lambda_2$, (2) there is a solution to the decision problem on $T$ for any $\lambda \le \lambda_1$, and (3) for {\em most} of the fragments, the distance between any two nodes is either smaller or equal to $\lambda_1$ or larger or equal to $\lambda_2$. This is achieved by applying Frederickson's parametric search on the monotone matrices capturing the pairwise distances between nodes in the same fragment. The (few) fragments that do not satisfy property (3) are handled naively in $O(b)$ time during query time. 
...
Due to space constraints, all  missing proofs are differed to Appendix \ref{appendix missing proofs}.
...
$b$. We perform preprocessing so that in query time we can handle most of the fragments in sublinear $O(\log b)$ time.  The overall preprocessing time
is $O(n\log b)$ plus $O(\log b)$ calls to the linear-time feasibility test.
Then, we show how to solve the unweighted dispersion problem with $O(\log^{2}n)$ calls to this sublinear
...
Each fragment is connected to the rest of the tree by at most two boundary nodes $u$ and $v$, such that $v$ is a
descendant of $u$, and the fragment consists of the subtree of $u$ without the subtree of $v$ ($v$ does not belong to
the fragment). See Figure~\ref{fig:fragment} in the appendix. We call the path from $u$ to $v$ the fragment's \textit{spine}, and $v$'s subtree its \textit{hole}. If the
...
To solve the optimization problem, we will maintain an interval $[\lambda_1,\lambda_2)$ such that
the value we are searching for in the optimization problem, $\lambda^{*}$, is inside the interval. That is, $\lambda_{1}$ is feasible
while $\lambda_{2}$ is not. In the beginning, the interval is simply $[0,\infty)$. In the preprocessing stage,
we would like to  sufficiently decrease the initial interval, so that for any $\lambda\in [\lambda_{1},\lambda_{2})$ and any two
nodes $u_{1}$ and $u_{2}$ in the same fragment either $d(u_{1},u_{2})\leq \lambda_{1}$ or $d(u_{1},u_{2})\geq\lambda_{2}$
and similarly $d(u_{1},u_{2})\leq \frac{\lambda_{1}}{2}$ or $d(u_{1},u_{2})\geq \frac{\lambda_{2}}{2}$
(and $\lambda^{*}\in [\lambda_{1},\lambda_{2})$ still holds). This will then allow us to preprocess the fragment,
so that for any possible $\lambda$ specified in query time we can process it in sublinear time.
We will achieve this goal for {\em most} of the fragments using  $O(\log b)$ calls to the linear-time feasibility test.
%Choosing which tests to perform to achieve this is the gist of Frederickson's method.

For each fragment, we construct an implicit representation of $O(b)$ sorted matrices of total side length $O(b\log b)$
(where a matrix is sorted if it is both row- and column-sorted), s.t. any pairwise distance (and any pairwise distance multiplied by 2) inside the fragment is an entry in some matrix. This is done using the standard centroid 
decomposition, in $O(n/b\cdot b\log b)=O(n\log b)$ total time using the following lemma. 
...
\end{restatable}
We then apply the following theorem by Frederickson on the obtained set of matrices.
Setting $m=b \log b \cdot \frac{n}{b} = n \log b$ and $p=n/b^2$, the theorem implies that we can use $O(\log b)$ feasibility tests and discard all but $n/b^2$ elements of the matrices.
...
We say that a fragment is {\em inactive} if any pairwise distance (and any pairwise distance multiplied by 2) inside it is either at most
$\lambda_{1}$ or at least $\lambda_{2}$. A fragment that is not inactive is called {\em active}.
Recall that we have at most $n/b^{2}$ active fragments and that we pay $O(n \log b)$ time exclusive of the feasibility tests.
For any two nodes in an inactive fragment, we can compare their distance to $\lambda$ and $\lambda/2$, for any possible $\lambda$ (i.e. any $\lambda \in (\lambda_1,\lambda_2)$). This allows us to preprocess each inactive fragment in $O(b)$ time as follows:

\begin{noindlist}
\item\textbf{Reduce the fragment to a caterpillar:}
a fragment consists of the spine and the subtrees hanging off the spine. Because we can compare any pairwise distance in the fragment to $\lambda$ and $\lambda/2$, we can run our linear-time feasibility test on the subtrees hanging off the spine, and obtain
...
and below. We delete all candidate nodes for which one of these distances is smaller or equal to $\lambda_{1}$.
...
\textbf{Precompute for any candidate node with respect to the hole:}
...
such a test is actually done by comparing to $\lambda_{2}$). Due to distances of the leaves to the hole
...
If the current fragment is active, we process it naively in $O(b)$ time (note that now we know $\lambda$, and so are
able to run the linear-time feasibility test). Otherwise, we will process the current fragment in $O(\log b)$ time.
...
In this section we present an $O(n\log n)$ time algorithm for the weighted decision problem (a matching lower bound follows by a reduction from the set disjointness problem as explained in Appendix \ref{appendix weighted f.t. lower bound}).
...
The algorithm computes such a polyline for the subtrees rooted at all the nodes of the tree bottom-up. In each step of the algorithm, we compute the polyline for a subtree rooted at some node $v$, assuming that we have already computed the polylines for the subtrees rooted at $v$'s children. W.l.o.g. we assume that the input tree is binary, and show how to
...
When constructing a polyline, we need to consider several cases, as the closest chosen node $h$ in the subtree of $v$ could be $v$ itself, or it could be in the subtree of one of $v$'s children. This means that for any key $d(h,v)$ in the polyline, the value for that key can represent a solution $P$ in which $h$ is in the subtree rooted at $v$'s left child, or in the subtree rooted at $v$'s right child, or it could be $v$ itself.
...
%
We begin with computing the value of $p$ for key zero. Consider the case where $v$ is in the solution (i.e., $h=v$). In this case we query $p_1$ and $p_2$ for their values with keys $\lambda - d(v,u_1)$ and $\lambda - d(v,u_2)$ respectively (if one of these is negative, we take zero instead), and add them together with the weight of $v$. Next consider the case where $v$ is not in the solution. %(i.e. there is a  subset with greater weight for the subtree rooted at $v$ that does not include $v$). 
...
There is an additional technical difficulty in Operation~\ref{op6}, as the intervals $(k_{i'},k_{i})$ might not be disjoint in our algorithm.
We do this operation more carefully and argue that the intervals can be made
disjoint with some extra work. The full description of Operation 6-8 is in Appendix \ref{appendix batched operations}.
