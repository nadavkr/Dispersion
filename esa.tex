...
developing a feasibility test that requires {\em linear} preprocessing and can then be queried in \emph{sublinear} time. This was the approach of Fredrickson for $k$-partitioning and $k$-center. It is plausible that his framework can be adapted to solve our problem as well, however, this to date has not been done. Our (slightly different) take on $k$-dispersion allows us to develop a  simpler and more structured algorithm, and hence might be of independent interest.
...
The representation consists of matrices where both rows and columns are sorted. Then, we run feasibility tests with a few values from 
...
in Appendix \ref{appendix constructing sorted matrices using centroid decomp.} in $O(n/b\cdot b\log b)=O(n\log b)$ total time.
Then, intuitively, by carefully choosing an entry in one of
...
matrices are row- and column-sorted. More precisely, we use the following theorem:
...
We store the certain node that is nearest to the root, the certain node that is nearest to the hole and the total number of certain nodes,
