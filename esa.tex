...
The focus of this paper is the  {\em $k$-dispersion} problem, where we wish to designate $k$ nodes as facilities so as to maximize the distances among the facilities. More formally, let $d(u,v)$ denote the distance between nodes $u$ and $v$, and for a subset of nodes $P$ let $f(P)=\min_{u,v\in P} \{d(u,v)\}$.
\begin{itemize} 
...
\end{itemize}
In other words, we wish to select $k$ nodes that are as spread-apart as possible. The dispersion problem be seen as a generalization of the classical maximum independent set problem (that can be solved by binary searching for the largest value of $k$ for which the minimum distance is at least 2).  
%
...
The optimization problem can be solved by repeatedly querying a {\em feasibility test} that solves the search version of this problem. 
\begin{itemize}
...
In this section we present an $O(n\log n)$ time algorithm for the weighted search problem (a matching lower bound follows by a reduction from the set disjointness problem as explained in Appendix \ref{appendix weighted f.t. lower bound}).
