...
nearest chosen node to $r$. Then, due to distances of the leaves to $r$ being non-decreasing, it is clear that
...
for any $j=1,2,\ldots,i'$. Therefore, to continue the simulation we should repeat the reasoning $u_{1},u_{2},u_{i'}$.
This suggest the following implementation: can the caterpillar from top to bottom and store, for every prefix $u_{1},u_{2},\ldots,u_{i}$,
the number of chosen nodes, the certain node and the candidate node. While scanning we maintain $i'$,
it is easy to see that after increasing $i$ we only have to keep increasing $i'$ as long as $d(u_{i},u_{i'})<\lambda$.
To store the information for the current prefix, we copy the already computed information for $u_{1},u_{2},u_{i'}$
and increase the number of chosen nodes by one. Then, if the candidate node is set to NULL and $d(r,u_{i})<\lambda/2$
we set the candidate node to $u_{i}$, and else similarly if the certain node is set to NULL we set the certain node to $u_{i}$.
We spend only amortized constant time per $i$, so the total preprocessing time is $O(b)$.
\end{enumerate}
