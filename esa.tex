...
In Section~\ref{sectionLinear} we give the high-level ideas of our optimal $O(n)$ time feasibility test (the full description of the algorithm is in Appendix \ref{appendix linear algorithm for the dispersion optimization problem}).
...
A proof of correctness for this algorithm can be found in Appendix~\ref{appendix proof of correctness for linear f.t.}.
...
\end{lemma}
A constructive proof for the lemma can be found in Appendix \ref{appendix proof of basic partitioning lemma}. The partitioning is done in $O(n)$ time.
...
problem with $O(\log n)$ calls to the feasibility tests, so in $O(n\log^{2}n)$ total time\footnote{As we mentioned in the introduction, it is also possible to use Frederickson's $k$-th path algorithm \cite{Frederickson1983}, as was done by Bhattacharya and Houle \cite{Bhattacharya1991,Bhattacharya1999}, and achieve the same running time.}.
...
The proof of the lemma can be found in Appendix~\ref{appendix proof of reduction lemma}.


\subsection{An \texorpdfstring{\boldmath$O(n \log ^2 n)$}{O(nlog^2n)} algorithm for the weighted feasibility test}
We now present an $O(n \log ^2 n)$ time algorithm for the weighted feasibility test, upon which we will base our optimal $O(n \log n)$ time weighted feasibility test.
...
We now describe the implementation of each operation. We denote the number of breakpoints of $p_1$ (the smaller polyline) by $x$, and the number of breakpoints of $p_2$ (the larger polyline) by $y$ (i.e. $x \leq y$). Note that the input for all four batched operations is given in sorted order (by keys).
...
The proof of the lemma can be found in Appendix~\ref{appendix proof of nlog^2n lemma}.
...
Once again we split our tree into small trees. We then delete the relevant intervals from each of the small trees. Each interval is either contained in a single small tree (and is then deleted by performing one or two split operations and at most one join operation) or it spans several small trees (in which case it is deleted by performing two split operations on the trees containing the interval's endpoints, and maybe deleting some trees entirely). Similarly to Operation~\ref{op5}, this costs $O(x \log(\frac{y}{x}) +x) = O(x \log(\frac{2y}{x}))$ in total.
...
\begin{proof}[\unskip\nopunct]
We will prove the correctness of the algorithm by proving that Invariant \ref{Maximality of P and distance of closest node invariant} holds for every step of the algorithm.
...
\subsection{Proof of Lemma~\ref{lemma of the reduction to set disjointness}}\label{appendix proof of reduction lemma}
\begin{lemma} %TODO change the lemma number
$X \cap Y = \emptyset$ iff the weighted feasibility test returns false for $T$ with  parameters $W=K$ and $\lambda =K$.
\end{lemma}
...
\subsection{Proof of Lemma~\ref{nlog^2n running time for weighted f.t. lemma}}\label{appendix proof of nlog^2n lemma}
\begin{lemma} 
The above implementation implies an $O(n \log ^2 n)$ weighted feasibility test.
\end{lemma}
...
The above implementation implies an $O(n \log n)$ weighted feasibility test.
\end{theorem}
The proof of the theorem can be found in Appendix 
\begin{proof}
