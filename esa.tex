...
%
It turns out that the $k$-dispersion and the $k$-partitioning problems are actually equivalent in the one-dimensional case (i.e., when the tree is a path),
by simply creating a new path with edges and nodes corresponding to nodes and edges, respectively, of the original path.
However, such equivalence does not apply to general trees, on which $k$-dispersion seems more difficult than $k$-partitioning. In particular, until the present work, no linear time solution for $k$-dispersion was known. The fastest solution was the $O(n \log n)$ solution given in the late 90's by Bhattacharya and Houle \cite{Bhattacharya1991}, who also considered the weighted version (where nodes also have weights)  for which they gave an $\Oh(n\log^4 n)$ time solution \cite{Bhattacharya1999}.
...
$P$ of nodes of total weight at least $W$. \cite{Bhattacharya1999} present an efficient feasibility test for this generalization, and by the same
...
Equipped with a sublinear feasibility test, it is still not clear how to solve the whole problem in $O(n)$ time, as in such complexity it is not trivial to represent all the pairwise distances in the tree in a structure that enables binary searching. To cope with this, we maintain % a current interval $[\lambda_1,\lambda_2)$ s.t. %$\lambda_1$ is the largest value we have tested and is feasible, and $\lambda_2$ is the smallest value we have tested and found not feasible (which implies that 
%the optimal value $\lambda^*$ lies within the interval, and generates,
%in every step, 
only a subset of candidate distances and represents them using matrices where both rows and columns are sorted.  Feasibility tests on only a few candidate entries from such matrices can eliminate many other candidates. Then, we prune the tree accordingly and repeat.
...
With some additional pruning, we can make sure that the distances of its leaves from both the hole and the root are monotonically increasing. This structure enables us to precompute for each candidate node, assuming this node is the bottom-most node
...
$b$ each. Each fragment is then preprocessed so that in query time we can handle it in sublinear $O(\log b)$ time.  The overall preprocessing time
...
and similarly $d(u_{1},u_{2})\leq 2\lambda_{1}$ or $d(u_{1},u_{2})\geq 2\lambda_{2}$
...
(where a matrix is sorted if it is both row- and column-sorted),
and any pairwise distance (and any pairwise distance multiplied by 2) inside is an entry in some matrix using the standard centroid 
...
Given a tree $T$ on $b$ nodes, we can construct in $O(b\log b)$ an implicit representation of $O(b)$ sorted matrices of total
...
We say that a fragment is {\em inactive} if any pairwise distance (and any pairwise distance multiplied by 2) inside is either smaller than
...
for every candidate node we find the nearest certain node. Then, we compare their distance to $\lambda$ and remove the
...
After increasing $i$, we only have to keep increasing $i'$ as long as $d(u_{i},u_{i'})<\lambda$.
...
we set the candidate node to $u_{i}$, and else if the certain node is set to NULL we set it to $u_{i}$.
\end{noindlist}
