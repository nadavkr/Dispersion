...
\cite{Bhattacharya1991} presents an efficient feasibility test, and uses a result by Fredrickson \cite{Frederickson1983} that enables binary searching over the possible values of $\lambda$ (which are all pairwise distances in the tree). In other words, a feasibility test with a running time $\tau$ implies an $O(n \log n + \tau \cdot \log n)$ time algorithm for the dispersion optimization problem. We present a simplified version of their linear time
feasibility test in Section~\ref{linear F.T.}.

In the weighted $k$-dispersion problem, nodes have non-negative weights and instead of $k$ we are given $W$. The goal is then to find a subset
$P$ of nodes of total weight at least $W$, s.t. $f(P)$ is maximized. \cite{Bhattacharya1999} present an efficient feasibility test for this generalization, and by the same
reasoning above solves the weighted dispersion optmization problem.
...
into larger and larger fragments. In each iteration we construct a feasibility test with better running time, until finally, after $\log ^*n$ iterations requiring a total of $O(n)$ time, we obtain a feasibility test with $O(\frac{n}{\log ^4n} \cdot \log \log n)$ query-time, which we use to solve the dispersion optimization problem in $O(n)$. The  algorithm is described in Appendix \ref{appendix linear algorithm for the dispersion optimization problem}.
...
for every possible distance $d$, we store the maximum $W(P)$ such that the distance of every node of $P$ to the root of the subtree is at least $d$.
...
any balanced search tree with split and join capabilities. Our data structure both simplifies and extends that of Brown and Tarjan~\cite{Brown1980}, and we believe it to be of independent interest. All missing proofs can be found in Appendix \ref{appendix missing proofs}.
...
$b$ each. We perform preoprocessing so that in query time we can handle most of the fragments in sublinear $O(\log b)$ time.  The overall preprocessing time
...
Each fragment is connected to the rest of the tree by at most two border nodes $u$ and $v$, such that $v$ is a
...
we would like to to sufficiently decrease the initial interval, so that for any $\lambda\in [\lambda_{1},\lambda_{2})$ and any two
...
(and $\lambda^{*}\in [\lambda_{1},\lambda_{2})$ still holds). Intuitively, this will then allow us to preprocess the fragment,
so that for any possible $\lambda$ specified in query time we can process it in sublinear time.
We will achieve this goal for {\em most} of the fragments using using $O(\log b)$ calls to the linear-time feasibility test.
...
decomposition, in $O(n/b\cdot b\log b)=O(n\log b)$ total time. Then, we can apply a theorem Frederickson on the obtained set of matrices.
...
We say that a fragment is {\em inactive} if any pairwise distance (and any pairwise distance multiplied by 2) inside is either smaller or equal to
$\lambda_{1}$ or larger or equal to $\lambda_{2}$. A fragment that is not inactive is called {\em active}.
By the choice of parameters, we have at most $n/b^{2}$ active fragments and we also pay $O(n \log b)$ exclusive of the feasibility tests.
For any two nodes in an inactive fragment, we can compare their distance to $\lambda$ and $\lambda/2$, for any possible $\lambda$ (i.e. any $\lambda$ s.t.  $\lambda \in (\lambda_1,\lambda_2)$). This allows us to preprocess each inactive fragment in $O(b)$ time as follows:
...
for each candidate node we find the nearest certain node. Then, we compare their distance to $\lambda$ and remove the
...
The algorithm computes such a polyline for the subtrees rooted at all the nodes of the tree bottom-up,
so in each step of the algorithm we compute the polyline for a subtree rooted at some node $v$, assuming that we have already computed the polylines for the subtrees rooted at $v$'s children. W.l.o.g. we assume that the input tree is binary, and show how to
implement this step in time $O(x \log (\frac{2y}{x}))$, where $x$ is the number of breakpoints in the polyline with fewer breakpoints, and $y$ the number of breakpoints in the other polyline.
...
Operation~\ref{op3} runs a predecessor query and returning the value at the returned breakpoint in logarithmic time.
...
Therefore, we need to consider how we use this operation more carefully and argue that the intervals can be made
