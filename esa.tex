...
fragment and run the linear-time feasibility test for all subtrees hanging off from it. Because of property (3), this can be done in advance without without knowing the actual exact value of $\lambda \in [\lambda_1,\lambda_2)$, which will only be determined at query time. 
Let $P$ be a solution produced by the feasibility test to a subtree rooted at a node $u$. The interaction between $P$ and the solution to the entire tree only depends on the distance from $u$ to its closet node in $P$, which we call {\emph the candidate} node. We can therefore conceptually prune each hanging subtree and think of the fragment as a caterpillar connecting the root and the hole. 
...
After some additional pruning, we can precompute information about the candidate nodes that will be used to accelerate queries to the feasibility test. During a query we will be able to jump over each fragment of size $O(b)$ in just $O(\log b)$ time, so the test takes $O(\frac{n}{b}\log b)$ time.
...
In contrast to the unweighted case, where the interaction of a subtree with the rest of the tree is captured by a single candidate node, in the weighted case each subtree might have a large number of candidates. 
...
\item If the distance from $r$ to the closet vertex in $P$ is at least $\lambda$, add $r$ to $P$.
\end{enumerate}
