...
\section{A Linear Time Unweighted Feasibility Test}
\label{linear F.T.}
...
\section{\texorpdfstring{\boldmath$O(n\log\log n)$}{O(nloglogn)} Time Algorithm for the Unweighted Dispersion Problem}
\label{sublinear f.t.}
...
is $O(\frac{n}{b}\cdot \log b)$. Then, we show how to solve the unweighted dispersion problem with $O(\log^{2}n)$ calls to the sublinear
...
\begin{figure}
\floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,center},capbesidewidth=9.7cm}}]{figure}[\FBwidth]
...
linear-time feasibility test to decrease the interval, so that {\em most} of the fragments have the property that any pairwise distance
...
and any pairwise distance inside is an entry in some matrix. This is done using the standard centroid decomposition as described
...
\section{A Linear Time Algorithm for the Unweighted Dispersion Problem}
\label{sectionLinear}
