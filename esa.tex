...
\end{restatable}
Then, we repeatedly choose an entry of a matrix and, by running a feasibility test, check if it is at most $\lambda^{*}$, which we call
{\em discarding} an element. Depending on the outcome, we then appropriately shrink the current interval $[\lambda_{1},\lambda_{2})$.
...
Setting $m=b \log b \cdot \frac{n}{b} = n \log b$ and $p=n/b^2$, the theorem implies that we can use $O(\log b)$ (linear time) feasibility tests and discard all but $n/b^2$ elements of the matrices. Therefore, all but at most $n/b^{2}$ fragments are inactive.
We next describe the process of precomputing the information for each non-active fragment. We choose $\lambda$ arbitrarily in
