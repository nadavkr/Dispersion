...
Both \cite{Bhattacharya1991} and \cite{Bhattacharya1999} present efficient feasibility tests, and use a result by Fredrickson \cite{Frederickson1983} that allows them to binary search over the possible values of $\lambda$ (which are all pairwise distances in the tree). This implies $O(n \log n + \tau \cdot \log n)$ time algorithms for the dispersion optimization problems (in both the weighted and unweighted cases), where $\tau$ is the running time of the feasibility test.
...
Again, this is the same as in Frederickson's approach, but our algorithm differs in which distances are represented in every step and how the
tree is pruned.
...
We first define a variant of the well known micro-macro decomposition that partitions the input tree $T$ into $O(n/b)$ {\em fragments}, each containing $O(b)$ nodes and at most two {\em boundary nodes} incident to nodes in other fragments: the root of the fragment and, possible, another boundary
node called the {\em hole}.
We use this to simulate a bottom-up feasibility test by jumping over entire 
fragments, i.e., knowing $\lambda$, we extend in $O(\log b)$ time a solution for a subtree of $T$ rooted at the fragment's hole to a subtree of $T$ rooted at the fragment's root. This is achieved by a preprocessing highlighted below.

The first step of the preprocessing computes values $\lambda_1$ and  $\lambda_2$ such that (1) there is no solution to the decision problem on $T$ for any $\lambda > \lambda_2$, (2) there is a solution to the decision problem on $T$ for any $\lambda \le \lambda_1$, and (3) for {\em most} of the fragments, the distance between any two nodes is either smaller than $\lambda_1$ or larger or equal to $\lambda_2$. This is achieved by
generating an implicit representation of the monotone matrices capturing the pairwise distance between the nodes in the same fragment
and then applying Frederickson's parametric search. The (few) fragments that do not satisfy property (3) are handled naively in $O(b)$ time during query time. 
...
as for any two nodes of the fragment we know if their distance is smaller than $\lambda_{1}$ or larger than $\lambda_{2}$. Then, we can conceptually
prune the fragment and think of it as a caterpillar. The spine of the caterpillar is the hole-to-root path and each spine node has two leaf children: a \emph{certain} node (that is guaranteed to be included in an optimal solution for any $\lambda$, and a \emph{candidate} node (whose inclusion in an optimal solution depends on $\lambda$). Finally, we perform additional pruning on the leaves so that their distances from both the hole and
the root are monotonically increasing. This structure enables us to precompute for each candidate node, assuming this node is the bottom-most node
...
Instead, we present a simpler approach for merging polylines in Section~\ref{O(nlogn) weighted f.t.} that efficiently supports the required batch operations and works with essentially any balanced search
tree with split and join capabilities. We believe that this is of independent interest due to its simplicity.

\section{A Linear Feasibility Test}\label{linear F.T.}

The feasibility test processes the tree bottom-up and computes, for every subtree $T_{r}$ rooted at a node $r$, a valid solution with maximal cardinality.
If $P$ is a valid solution for the subtree $T_{r}$ (i.e., $f(P)\geq\lambda$), we call a node $u\in P$, s.t. $d(r,u)<\frac{\lambda}{2}$, the \emph{candidate} node of the subtree (or a candidate with respect to $r$). There is at most one such candidate node.
...
In each step we are given a node $r$, its children nodes $r_{1},r_{2},\ldots,r_{k}$, and for each child $r_{i}$ we are given a maximal valid
solution $P_{i}$ for the feasibility test on $T_{r_{i}}$. We would like to produce a maximal valid solution for the feasibility test on $r$'s subtree.
Denote by $P_{1},\ldots,P_{k}$ the solutions for the feasibility test on each subtree rooted at a child of $r$. We obtain a maximal valid solution $P$
for the feasibility test on $T_{r}$ as follows:
\begin{enumerate}
\item Take all the vertices in $P_{1},\ldots,P_{k}$, except for the candidate vertices.
\item Take all candidate nodes $u$ s.t. $d(u,r) \geq \frac{\lambda}{2}$ (i.e., they are certain w.r.t. $r$).
\item If it exists, take $u'$, the candidate node farthest from $r$ s.t. $d(u',r) < \frac{\lambda}{2}$ and $d(u',x)\geq \lambda$, where $x$ is the closest node to $u'$ we have taken so far.
...
To accelerate the linear-time feasibility test described in Section~\ref{linear F.T.}, we want to partition the tree into fragments.
Then, the fragments are preprocessed so that in query time we can process them in sublinear time.
To this end, we need a specific way of partitioning the tree into fragments, where
each fragment is connected to the rest of the tree by at most two vertices and consists of at most $b$ nodes,
and there are only $O(n/b)$ fragments. Each fragment is defined by its border vertices $u$ and $v$, s.t. $v$ is a descendant of $u$, and consists of $u$'s subtree without $v$'s subtree ($v$ itself is not part of the fragment). We call the path from $u$ to $v$, the fragment's \textit{spine}, and $v$'s subtree the \textit{hole} of the fragment. In the case where the fragment has only one border vertex, i.e. the fragment is a subtree of the input tree, we say the fragment has no hole. We call such a partition of the tree a \emph{good partition}.
...
This is done with the parametric search method by Frederickson as follows. We maintain an interval $[\lambda_1,\lambda_2)$ and
$\lambda^{*}$ that we are searching for in the optimization problem is surely inside the interval, that is, $\lambda_{1}$ is feasibly
while $\lambda_{2}$ is not. In the beginning, the interval is simply $[0,\infty)$. Then, we use $O(\log\log n)$ calls to the
...
(and also any pairwise distance multiplied by 2) inside is either smaller than $\lambda_{1}$ or larger than $\lambda_{2}$.
...
and any pairwise distance inside is an entry is some matrix. This is done using the standard centroid decomposition as described
in Appendix \ref{appendix constructing sorted matrices using centroid decomp.}. Intuitively, by carefully choosing an entry in one of
these matrices and using the linear-time feasibility test, we can actually eliminate a large piece of the matrix, because the
matrices are row- and column-sorted. We use the following theorem:
...
$\lambda_{1}$ or larger than $\lambda_{2}$ \text{inactive}, and all other fragments \textit{active}.
...
For an inactive fragment, we can now check if the distance between two nodes inside is at least $\lambda$ or $\lambda/2$ by first calculating
the distance in $O(1)$ time (using lowest common ancestor queries~\cite{Bender2000}) and comparing it to $\lambda_{1}$.
...
\textbf{Find candidate nodes that cannot be taken:}\\
...
From now on we ignore the certain nodes, and only deal with the remaining candidate nodes.
...
and order the leaves so that $x_{1}<x_{2}<\ldots$. See Figure~\ref{fig:pruning} in the appendix.
...
\textbf{Prune leaves to make their distances to the hole non-decreasing:}\\
...
Out of all the already chosen nodes in the subtree rooted at $v$, let $h$ be the node for which $d(h,v)$ is minimized. We call $h$ the {\em closest chosen node} in $v$'s subtree. In our weighted feasibility test, $v$ will store an optimal solution $P$ for any possible value of $d(h,v)$ (up to $\lambda$, since if the closest chosen node is farther, it does not affect nodes outside the subtree). That is, a subset of nodes $P$ in $v$'s subtree of maximum weight $W(P)$ s.t. the closest chosen node is at distance {\em greater or equal} to $d(h,v)$ from $v$, and $d(x_1,x_2) \geq \lambda$ for every  $x_1,x_2\in P$. 
...
Assume w.l.o.g. that the number of breakpoints in $p_1$ is smaller or equal to the number of breakpoints in $p_2$\footnote{Note that if one of $v$'s children is at distance greater or equal to $\lambda$, the situation is simpler. Suppose that $d(v,u_1) \geq \lambda$. We can compute the polyline for $v$'s subtree as if it were a degree one node, ignoring $u_1$, and then simply increase all the values of the resulting polyline by the value of $p_1$ for key zero.}.
...
We start by splitting $p_1$ at $\frac{\lambda}{2}$ and taking the first half (denoted by $p_1'$). We then split $p_2$ at $\frac{\lambda}{2}$ and take the second half (denoted by $p_2'$). Consider two consecutive breakpoints of $p_1'$ with keys $x$ and $x+y$. For any key $x'$, s.t. $x \leq x' < x+y$, we would like to compute the value of $p_2'$ for the key $\lambda-x'$, and add it to the value of $p_1'$ for the key $x'$. Notice that here we query $p_2'$ with keys $y'$ s.t. $\lambda -x -y < y' \leq \lambda-x$. If we were to actually increase the values of $p_1'$ in such a way, we would obtain a polyline that is not monotonically decreasing, that is, does not account for the fact that for a key $x$ we want the maximum weight $W(P)$ s.t. the closest chosen node is at distance \emph{at leas}t $x$. Therefore, we would have to further prune the polyline by setting the value for each key $x$ to be the maximum value over all keys $x' \geq x$.
Instead, we can just as well increase the value of $p_{1}'$ in $[x,x+y)$ by the maximal value of $p_{2}'$ in $(\lambda-x-y,\lambda-x]$. Since $p_2'$ is monotonically decreasing, the maximal value will be at the minimal key. This value can be found by a predecessor query on $p_2'$ with key $\lambda-x-y$. Overall, we need to iterate over $p_1'$, and for every pair of consecutive breakpoints with keys $x$ and $x+y$, we add the value of $p_2'$ for key $\lambda-x-y+\epsilon$ to the value stored in the breakpoint of $p_1'$ with key $x$.
...
To complete the construction, we make the polyline monotonically decreasing by scanning it from $\frac{\lambda}{2}$ to zero and deleting unnecessary breakpoints. The time this takes is bounded by the number of breakpoints in $p_{1}$.
...
We next describe the implementation of each operation. We denote the number of breakpoints of $p_1$ (the smaller polyline) by $x$, and the number of breakpoints of $p_2$ (the larger polyline) by $y$ (i.e. $x \leq y$).
...
\begin{lemma}\label{running time to obtain small trees lemma}
...
% This can be done by scanning the list of breakpoints to be inserted from left to right, while scanning the roots of the small trees. 
...
\begin{theorem}\label{nlogn weighted f.t. theorem}
...
\subsection{Proof of correctness for the linear time feasibility test in the unweighted case}\label{appendix proof of correctness for linear f.t.}
...
produces $P$, a subset of the vertices of the subtree rooted at $r$, s.t. $f(P)\geq\lambda$ and $|P|$ is maximal. Out of all such valid subsets
with maximal cardinality, the distance between $r$ and the node closest to it is maximized in $P$. The proof is by induction.

Consider a node $r$ of the tree and and assume that the invariant golds for all of its children $r_{1},r_{2},\ldots,r_{k}$.
...
The whole process can be implemented by scanning the tree twice bottom-up in $O(n)$ time.
\end{proof}
...
\begin{lemma}
The time to obtain the small trees by splitting is $O(x \log (\frac{2y}{x}))$.
\end{lemma}
