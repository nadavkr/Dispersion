...
\subsection{An \texorpdfstring{\boldmath$ \Omega (n\log ^2 n)$}{O(nlog^2n)} time algorithm for the weighted feasibility test}\label{weighted f.t.}
...
We iterate over $p_a$ and for each breakpoint, we check if the value of $p_b$ for the same key is between the values of this breakpoint and the predecessor breakpoint in $p_a$. If so, this is an intersection point. Then, we find the intersection points which are between breakpoints of $p_a$, by running a value predecessor query on $p_b$ for every breakpoint in $p_a$ except for the first. %TODO elaborate in full version
%We iterate over $p_a$ and for every pair of consecutive breakpoints we check which polyline has the higher value at both points. If the answer for both points is the same, we carry on. Otherwise, there must be a single intersection point of the two polylines inside this interval. We find this intersection by running a value predecessor query on $p_b$, with the key being the right endpoint of the interval. 
