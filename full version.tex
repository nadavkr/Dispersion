...
We define the \emph{interesting prefix} of a caterpillar as all the candidate nodes s.t. there is no candidate above them in the caterpillar that is at distance at least $\lambda^*$ from them.
...
In order to preprocess the large caterpillar and compute the required information, we iterate over the small caterpillars from top to bottom, and process each small caterpillar. The following invariant holds:
\begin{invariant} \label{invariat small caterpillar is correct after prep.} After we preprocess a small caterpillar, each node in the interesting suffix if it stores the correct information for the solution where this node is the bottom-most chosen candidate in the large caterpillar (i.e. the number of chosen nodes, the candidate node, and the certain node). 
\end{invariant}

Let us describe the steps of the preprocessing of a small caterpillar (after which Invariant \ref{invariat small caterpillar is correct after prep.} holds):

\begin{itemize}
\item \emph{preprocessing the interesting prefix:}





\item \emph{preprocessing the interesting suffix:}
\end{itemize}
