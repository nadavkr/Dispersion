...
%\end{enumerate}
%
%It might be the case, that the certain node field stored for the current node of the interesting suffix points to a node below the interesting prefix of the small caterpillar. This only happens if the candidate node field points to a node that was removed in the previous step (due to Lemma \ref{lemma3}), due to the definition of an interesting prefix. In this case we must find the closest remaining candidate, which is above the removed node, in order to update the solution stored for the current node. For this reason, we do not actually remove nodes in the previous step (Subsection \ref{section:lemma3}), but instead only mark remove nodes, and maintain a pointer from each marked node to the next candidate node above it, that is not marked. In the end of each iteration we will sweep through the relevant nodes and remove the marked nodes (since only marked nodes can be removed due to property \ref{property unstable affix contained in interesting affix}. We now have the correct information stored for each node of the interesting suffix of the current small caterpillar, and continue to the next one.
