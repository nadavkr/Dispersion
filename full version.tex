...
\vspace{0.04in} \noindent {\bf The unstable prefix and unstable  suffix of a caterpillar.} 
...
If the interesting suffix is the entire caterpillar, then the interesting prefix is also the entire caterpillar. We call such a caterpillar \emph{fresh}.
...
an interesting prefix or suffix and call them \emph{relevant}. In particular,
...
Consider the interesting prefix of some small caterpillar. It consists of all the nodes for which the next chosen candidate in the large caterpillar is in a different small caterpillar. We would like to find for each node in the interesting prefix, the closest node above it which is at distance at least $\lambda^*$ from it. The node we seek is in the interesting suffix of the small caterpillar above the current one. If the above caterpillar is fresh, we also need to look at the interesting suffix of the small caterpillar above it (which might also be fresh, and so on). We iterate over the interesting prefix top to bottom, while iterating over the set of fresh caterpillars and interesting suffix where we need to search. For each node in the interesting prefix we find the closest node at distance at least $\lambda^*$ above it (if it exists), and update the required information using the information stored in the node we found. Note that we only look at relevant nodes here, and for each pair of relevant nodes we already know if their distance is smaller than $\lambda^*$ or not.

We now need to update the information stored at some of the nodes of the small caterpillar which are not in the interesting prefix. Notice that from now on the only nodes which we might query for information are the nodes of the interesting suffix of the small caterpillar, and so we only need to update them. We iterate over the interesting suffix, and for each node check if the solution stored in it has a candidate node. If so, we check if the node stored in the candidate node field was pruned in the previous step, and update accordingly. %TODO add more?
We then check if the information stored in the candidate node, or if it does not exist, the certain node, was updated when we processed the interesting prefix, and if so update the current node accordingly. 
We have the following:
\begin{enumerate}
\item Due to Lemma \ref{stable infix} any pruned node was either in the suffix of the small caterpillar, in which case it is not part of the stored solution for any remaining node, or it was in the unstable prefix of the small caterpillar, and so if it is part of the solution stored for some remaining node, it must be the candidate node of the solution.
