...
\begin{itemize}
\item \ If $y$ was pruned because it was closer to the root than some node above it (which we call the {\em removing node}), then the removing node cannot be above $z$. This is because otherwise it would imply that either $z$ should also be pruned, or that the removing node is not the cause for $y$'s pruning.
