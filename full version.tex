...
Given a binary tree on $n$ nodes, there is a linear time algorithm for constructing a hierarchy of $O(\log^*n)$ good partitions of the tree with the following properties:
\begin{itemize}
\item The partition at level $\ell$ of the hierarchy has $O(n/{{b_{\ell}}^4})$ fragments, each of size $O({b_{\ell}}^5)$.
\item $b_\ell = 2^{b_{\ell -1}}$.
\item Every fragment in the partition at level $\ell-1$ of the hierarchy is contained inside a fragment in the partition at level $\ell$.
\item At the first level of the hierarchy $b_{\ell}$ is a constant, and at the last level of the hierarchy $b_{\ell} = \Theta(\log n)$.
\end{itemize}
...
node and a candidate node, we only have to consider the remaining candidate nodes. For convenience, we will refer to the remaining candidate nodes (attached to the spine) simply as nodes, and reserve the term {\em candidate} for the nodes stored as candidate nodes of solutions in the precomputed information.
...
We therefore store the nodes of each caterpillar in a balanced search tree keyed by the node's distance to the hole of the caterpillar. Note that, even though each search tree is keyed by distances to a different hole, we can still merge two trees in logarithmic time. This is because they correspond to two small caterpillars where one is above the other (and hence all its keys can be thought of as larger than all the keys of the bottom one). Since we merge multiple search trees of small caterpillars into one search tree of a large caterpillar, each merge operation costs at most $O(\log B)$, and we have $O(n/b^9)$ such operations (one per small caterpillar), this sums up to $O(\frac{n}{b})$ and therefore to $O(n)$ overall.
...
The main goal of the current step is to update the information stored in each node (i.e. the certain node, the candidate node, and the number of chosen nodes) for any node that might be queried in the future, so that the information holds the solution for the large caterpillar. When doing this, we need to take into account that the information that is currently stored in nodes might be false, even with regard to the appropriate small caterpillar, since we have eliminated some nodes in the previous step.
We next define a few terms and a lemma that will be used in the rest of this section.

\vspace{0.04in} \noindent {\bf The unstable and interesting affixes.} 
...
\emph{unstable suffix} to consist of all nodes $u_{i}$ such that $d(h,u_{i}) < \frac{\lambda^{*}}{2}$.
...
fragments. If we discover that a large caterpillar is fresh, then there is nothing to update. 
...
We iterate over the interesting prefix of the current small caterpillar top to bottom. % while iterating over the set of fresh caterpillars and interesting suffixes where we need to search. 
For each node in this interesting prefix, we find the closest node at distance at least $\lambda^*$ above it (if it exists), and update the information stored in the current node of the interesting prefix, using the information stored in the node we found (i.e. add the number of chosen nodes, and copy the certain node and the candidate node stored in it). Note that we only look at relevant nodes here, and for each pair of relevant nodes we already know whether the distance between them is smaller than $\lambda^*$ or not.
...
We check if the information stored in node $x$ was updated when we processed the interesting prefix. If it was updated, then we update the information stored in the current node (of the interesting suffix) accordingly, and move on to the next node in the interesting suffix. 

If however, the information stored in node $x$ was not updated, then $x$ is not in the interesting prefix of the current small caterpillar, which implies (by definition of the interesting prefix) that it is the certain node stored for this solution. Moreover, since $x$ does not belong to the interesting prefix, there was a node $y$ above $x$ in the current small caterpillar which was at distance greater or equal to $\lambda^*$ from $x$. Therefore, $y$ had been stored as the candidate node of the solution, and was pruned due to Lemma \ref{lemma3}. Hence, we need to find the bottom-most remaining node above $x$ which is at distance greater or equal to $\lambda^*$ from it. We claim that this node is simply the bottom-most remaining node above the current small caterpillar. Denote this node by $z$. We show that $d(x,z) \ge d(x,y)$ (implying that $d(x,z) \ge \lambda^*$).
We consider the three possible cases:
\begin{itemize}
\item \ If $y$ was pruned because it was closer to the root than some node above it (which we call the {\em removing node}), then the removing node cannot be above $z$. Because this would imply that either $z$ should have also be pruned, or that the removing node was not the cause for $y$'s pruning.
...
\item \  If $y$ was pruned due to a collision with some certain node, then in the process of pruning nodes so they are sorted by their distance to the hole (which is done before the collision elimination) $y$ and $z$ were not pruned. This implies that $z$ is farther from the hole than $y$, and so it is also farther from $x$.
...
It remains to analyze the running time of this preprocessing procedure. We claim that it amortizes to $O(n)$. 
