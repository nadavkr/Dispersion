...
We need to store the caterpillars in a data structure that will allow us to both merge caterpillars efficiently, and search for the bottom-most candidate in a caterpillar which is at distance greater or equal to $d$ from the hole, for any given $d$ s.t. $d \le \lambda^*$. We therefore store the candidates of each caterpillar in a balanced search tree ordered by the distance to the hole of the caterpillar. Note that when we merge two such trees, each tree is ordered by the distance to another node (the two small caterpillars have different holes), but the order of each search tree is the same is the order by the distance to the hole of the large caterpillar. Since we merge the balanced search trees of small caterpillars into one large tree for each large caterpillar, each merge operation costs at most $O(\log B)$, and we have $O(n/b^9)$ such operations (one per small caterpillar), this sums up to $O(\frac{n}{b})$ and $O(n)$ overall.
...
The main goal of this step is to update the information stored in each node (i.e. the certain node, candidate node, and number of chosen nodes) for any node that might be queried in the future, so that the information matches the solution for the large caterpillar. When doing this, we need to take into account that the information that is currently stored in nodes might be false, even with regard to the appropriate small caterpillar, since we have eliminated some nodes in the previous step.
...
We define the \emph{interesting prefix} of a caterpillar as all the candidate nodes whose distance from every candidate node above them is less than $\lambda^*$.
...
If the interesting suffix is the entire caterpillar, then by definition, the interesting prefix is also the entire caterpillar. We call such a caterpillar \emph{fresh}. Note that these definitions immediately imply that the unstable prefix (suffix) of any caterpillar is contained in its interesting prefix (suffix).


We now describe the preprocessing done on a given large caterpillar. We start by checking if the large caterpillar is fresh, i.e. if the distance between the bottom-most candidate and the top-most candidate is smaller than $\lambda^*$. This can be
...
fragments. If a large caterpillar is fresh, there is nothing to update. 
...
In order to preprocess the large caterpillar and compute the required information, we iterate over the small caterpillars from top to bottom, and process each small caterpillar. We will show that the following invariant holds:
\begin{invariant} \label{invariat small caterpillar is correct after prep.} After we preprocess a small caterpillar, each node in its interesting suffix stores the correct information for the solution where this node is the bottom-most chosen candidate in the large caterpillar. 
\end{invariant}

Let us describe the preprocessing of a small caterpillar:
...
Since Invariant \ref{invariat small caterpillar is correct after prep.} holds for every small caterpillar above the current one, we now have the correct information stored for every node of the interesting prefix.
...
\begin{lemma}
Invariant \ref{invariat small caterpillar is correct after prep.} holds.
\end{lemma}
\begin{proof}
...
%Consider the interesting prefix of some small caterpillar. It consists of all the nodes for which the next chosen candidate in the large caterpillar (i.e. the closest candidate node above the current one, which is at distance greater or equal to $\lambda^*$ from it) is in a different small caterpillar. We would like to find for each node in the interesting prefix, the closest node above it which is at distance at least $\lambda^*$ from it. The node we seek is in the interesting suffix of the small caterpillar above the current one. If the above caterpillar is fresh, we also need to look at the interesting suffix of the small caterpillar above it (which might also be fresh, and so on). We iterate over the interesting prefix top to bottom, while iterating over the set of fresh caterpillars and interesting suffix where we need to search. For each node in the interesting prefix we find the closest node at distance at least $\lambda^*$ above it (if it exists), and update the the information stored with the current node of the interesting prefix, using the information stored in the node we found (i.e. add the number of chosen nodes, and copy the certain node and candidate node stored in it). Note that we only look at relevant nodes here, and for each pair of relevant nodes we already know whether the distance between them is smaller than $\lambda^*$ or not.

%We now need to update the information stored at some of the nodes of the small caterpillar which are not in the interesting prefix. Notice that from now on the only nodes of the current small caterpillar that we might query for information are the nodes of the interesting suffix (since any other node cannot be the bottom-most node that is far enough from the hole), and so we only need to update them. We iterate over the interesting suffix, and update the information as follows. Consider a node of the interesting suffix. Denote the node stored in the candidate node field of this node by $x$, and if there is no candidate node, let $x$ denote the certain node stored there. Note that any candidate node that was removed due to Lemma \ref{lemma3}, and was part of a solution stored in some candidate node,  must be the candidate node of the solution or the node where the information is stored, due to Lemma \ref{stable infix}, so this makes sure we do not take into account nodes that were removed in the previous step. We check if the information stored in node $x$ was updated when we processed the interesting prefix. If it was updated, we update the information stored in the current node (of the interesting suffix) accordingly, and move to the next node in the interesting suffix. If it was not updated, then $x$ is not in the interesting prefix of the current small caterpillar, which implies that it is the certain node stored for this solution (due to the definition of the interesting prefix). Moreover, since $x$ does not belong to the interesting prefix, there was a candidate node above $x$ in the small caterpillar which was at distance greater or equal to $\lambda^*$ from $x$, and this node (denoted by $y$) had been stored as the candidate node of the solution, and was removed due to Lemma \ref{lemma3}. Therefore, we must find the next remaining candidate node attached to the spine of the large caterpillar which is at distance greater or equal to $\lambda^*$ from $x$. We claim that this node is simply the next remaining candidate above the current small caterpillar. For this to be true, the pruning and elimination process performed in Subsection \ref{section:lemma3} must start with pruning the candidate nodes s.t. their distances from the hole are monotone, and only then prune s.t. their distances to the root are monotone and there are no collisions with certain nodes. This will ensure that any candidate node above the current small caterpillar is farther away from $x$ than $y$, and specifically its distance from $x$ is at least $\lambda^*$, and so we should take the first remaining candidate above $y$. Note that since we always remove an entire prefix or suffix during the pruning process, the first remaining candidate node above $y$ must be in a different small caterpillar. We update the information in the current node according to the information stored in that node, proceed to the next node in the interesting suffix, and update it in the same manner.
